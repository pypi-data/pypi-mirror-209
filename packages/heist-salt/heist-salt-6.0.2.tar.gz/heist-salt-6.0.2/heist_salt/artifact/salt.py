...
async def update(
    ...
    for path in [old_binary, new_binary]:
        if not hub.tool.path.clean_path(artifacts_dir, path):
            hub.log.error(
                f"The binary path {binary} is not a valid path when attempting to update"
            )
            return False
    ...
    target_new_binary = run_dir / pathlib.Path(new_binary).name
