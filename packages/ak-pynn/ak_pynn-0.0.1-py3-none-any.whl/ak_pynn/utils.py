...
def elu(z, derive=False,elu_alpha=0.1):
    ...
    z=np.clip(z,a_min=-700,a_max=700)
    if (derive):
        return np.where(z > 0, 1, z+elu_alpha)
    else:
        return np.where(z > 0, z, elu_alpha*(np.exp(z)+elu_alpha))
...
def leaky_relu(z, derive=False,leaky_relu_fraction=0.01):
    ...
    if (derive):
        ...
        return np.max(leaky_relu_fraction*z, z)
...
def mae(y,p):
    ...
    """
    return np.mean(np.mean(np.abs(y - p),axis=1))
...
def mse(y, p):
    ...
    """
    return np.mean(np.mean((y - p) ** 2,axis=1))
...
def binary_cross_entropy(y, p):
    ...
    """
    return np.mean(np.mean(-(y*np.log(p)+(1-y)*np.log(1-p)),axis=1))
