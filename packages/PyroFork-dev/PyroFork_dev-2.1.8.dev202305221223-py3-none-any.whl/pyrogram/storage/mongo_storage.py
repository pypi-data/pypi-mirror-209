...
class MongoStorage(Storage):
    ...
    Parameters:
        ...
        - db (:obj:`~async_pymongo.AsyncClient`, :obj:`~motor.AsyncIOMotorClient`, *optional*):
    ...
    Example:
        ...
        db = async_pymongo.AsyncClient("mongodb://...")
        bot_db = db["my_bot"]
        session = MongoStorage("my_session", db=db, remove_peers=True)
    """
    lock: asyncio.Lock
    ...
    def __init__(
        ...
        name: str,
        uri: str = None,
        db: Union["AsyncClient", "AsyncIOMotorClient"] = None,
        remove_peers: bool = False
    ):
        super().__init__(name=name)
        if (
            isinstance(db, AsyncClient)
            or
            isinstance(db, AsyncIOMotorClient)
        ):
            database = db[name]
        elif uri:
            database = AsyncClient(uri)[name]
        else:
            raise Exception("Sorry, you need to fill one of following parameter!: uri, db") 
