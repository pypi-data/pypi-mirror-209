...
def highlight(
    ...
    is_file = filename.rpartition(".")[0]  # an actual filename, eg "date_util.py"
    if not is_file:
        filename = f"XaXaX{filename}"
    # if diff:
    ...
    if focus:
        ...
        for focus_line in focus:
            # XXX given_id = f"{random_id}-L{focus_line}".format(random_id, focus_line)
    ...
    if diff:
        ...
        if not is_file:
            html = html.replace(f"{filename}-", "L")
    return html
