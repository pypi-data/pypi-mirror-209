        ...
        if not root:
            ...
            if root.left == None and root.right == None:
                ...
                if query < currRoot:
                    ...
                    T1 = root.right
                    T2 = root.left
                print("query: ", query, "currRoot: ", currRoot)
                
                self.nnKDTreeRec(queryPoint, T1, threshold, noOfPoints, listOfNeighbors)
                print("list of neighbors after T1: ", listOfNeighbors)
                print("farthest neighbor: ", listOfNeighbors[-1][0],"distance= ", distance, distance <= threshold, listOfNeighbors[-1][0] <= distance)
                if distance <= threshold and (len(listOfNeighbors) < noOfPoints or listOfNeighbors[-1][0] > self.getDistance(queryPoint, T2.val)):
                    print("calling NN with T2", T2.val)
                    self.nnKDTreeRec(queryPoint, T2, threshold, noOfPoints, listOfNeighbors)
                print("list of neighbors after T2: ", listOfNeighbors)
        return listOfNeighbors