...
class TaskRun(Object):
    ...
    def logs(self):
        ...
        req = GetLogsForRunRequest(self._pod_id)
    ...
    def _repr_html_(self):
        ...
        return f"""
            <table>
              ...
              <tr><td>Created At</td><td>{self._created_at}</td></tr>
              <tr><td>Updated At</td><td>{self._updated_at}</td></tr>
            </table>
            """
    ...
    def _repr_html_(self) -> str:
        def task_run_row(task_run: TaskRun) -> str:
            ...
            <tr>
              ...
              <td>{task_run._created_at}</td>
              <td>{task_run._updated_at}</td>
            </tr>
            """
...
class TaskRunLogs(List[str]):
    ...
    def _repr_html_(self):
        def log_row(log: str):
            split = log.split(" ")
            time = dateparse.isoparse(split[0])
            channel = split[1]
            arr = split[2:]

            return f"""<tr style="text-align: left">
                  <td nowrap>{time.strftime("%Y-%d-%b %H:%M:%S")}</td>
                  <td>{channel}</td>
                  <td>{" ".join(arr)}</td>
            """
