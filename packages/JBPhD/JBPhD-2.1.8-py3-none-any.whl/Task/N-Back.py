...
filename = os.path.join(spwf.folder_name, f"{participant_number} - N-Back Score.csv")
filename2 = os.path.join(spwf.folder_name, f"{participant_number} - N-Back Reaction Times.csv")
...
pygame.font.init()# Set up the font
FONT_SIZE = 240
...
def draw_text(text, position, color):
    ...
    text_rect = text_surface.get_rect(center=position)  # center the text rectangle on the given position
