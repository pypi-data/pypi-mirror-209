    ...
    def update_device_type_eox_data(self, pid, eox_data):
        ...
        try:
            ...
            if "Incorrect PID:" in eox_data["EOXRecord"][0]["EOXError"]["ErrorDescription"]:
                eox_error = f"No EOX Support Statement Available"
                self.stdout.write(self.style.SUCCESS("%s - No EOX Support Statement Available" % pid))
            else:
        ...
        # Do nothing when JSON field does not exist
            ...
            self.stdout.write(self.style.SUCCESS("%s  - EOX Support Statement Available" % pid))
        ...
        try:
            if not eox_data["EOXRecord"][0]["LastDateOfSupport"]["value"]:
                ...
                self.stdout.write(
                    self.style.SUCCESS("%s - last_date_of_support: %s" % (pid, last_date_of_support))
                )
        ...
        try:
            if not eox_data["EOXRecord"][0]["EndOfSvcAttachDate"]["value"]:
                ...
                self.stdout.write(
                    self.style.SUCCESS("%s - end_of_svc_attach_date: %s" % (pid, end_of_svc_attach_date))
                )
