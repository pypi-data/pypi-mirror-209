...
class ImageApi:
    ...
    def build_url(
        ...
        dev_api_url, prod_api_url = ImageApi._get_image_api_urls()
        if url_orig.startswith(prod_api_url) or url_orig.startswith(dev_api_url):
            return url_orig
    ...
    def _get_image_api_urls():
        dev_url = current_app.config.get('DH_POTLUCK_DEV_IMAGE_API_URL', DEFAULT_DEV_IMAGE_API_URL)
        prod_url = current_app.config.get(
