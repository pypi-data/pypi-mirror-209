...
class QueryBuilder():
    ...
    def compile(self, defaultName=None, isChild=False, cursor: Optional[str] = None) -> str:
    ...
    def compile_plural_paginated(self,
        ...
        _query = _query.replace('$Q_NAME', query_name)
        # cursors changed to strings (like "g3QAAAABZAACaWRhEA") instead of ints
        _query = _query.replace('$CURSOR', f'"{cursor}"' if cursor is not None else 'null')
