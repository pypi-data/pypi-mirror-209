...
class CensiusClient(object):
    ...
    def register_dataset(self, *args, **kwargs):
        ...
        if missing_columns:
            return {
                "error": f"Missing columns in the input dataframe: {missing_columns}"
            }

        # 1.A Check if there are any string columns or mis matched columns
        # errors_list = dataset_validation.check_for_string_in_df(
        #     file, kwargs["features"]
        # )
        # if errors_list:
        #     return {"error": errors_list}
        ...
        if cat_map:
            (
                error_flag,
                error_list,
            ) = dataset_validation.validate_categorical_feature_from_input(file, cat_map)
    ...
    def bulk_log(self, input, **kwargs):
        ...
        """
            ...
            3. Check if input dataframe has any empty row in the middle
        """
        error_flag, error_message = bulk_input_validation.check_for_input_dataframe(
            input
        )
        if error_flag:
        ...
        input[kwargs["prediction_id_column"]] = input[
            kwargs["prediction_id_column"]
        ].astype(str)
        ...
        if "explanations" in kwargs.keys():
            try:
                ...
                validate(
                    instance=explanations_tabular, schema=explanations_tabular_schema
                )
        ...
        if (
            not actuals_received
            and not predictions_received
            and not explanations_received
        ):
        ...
        if predictions_received:
            if "features" in prediction_tabular.keys():
                ...
                missing_columns = (
                    bulk_input_validation.check_provided_column_with_input_columns(
                ...
                provided_feature_columns.remove(
                    prediction_tabular["prediction_confidence_column"]
                )
        ...
        if response.status_code == 200:
            ...
            registered_dataset_to_datatype = registered_dataset_details[
                "dataset_datatype"
            ]
        ...
        if predictions_received or actuals_received:
            ...
            model_details = {
                ...
                "actualColumn": input_map[actuals_column]
                if actuals_received
                else "None",
            }
            ...
            # print(input_map)
            # process_bulk
            response = bulk_input_processing.process_bulk(
    ...
    def update_model_iteration(self, **kwargs):
        ...
        try:
            ...
            validationError = (
                e.schema["errorMessage"] if "errorMessage" in e.schema else e.message
            )
    ...
    def __individual_log(self, *args, **kwargs):
        ...
        response = requests.request(
            "POST", LOG_URL(self.project_id), headers=headers, data=payload
        )
    ...
    def update_actual(self, *args, **kwargs):
        ...
        response = requests.request(
            "POST",
            UPDATE_ACTUAL_URL(self.project_id, kwargs["prediction_id"]),
            headers=headers,
            data=payload,
        )
        return self.__return_message(response)
