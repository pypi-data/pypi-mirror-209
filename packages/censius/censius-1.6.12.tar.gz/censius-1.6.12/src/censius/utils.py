...
class DataSetValidation:
    ...
    def check_for_alphanumeric_features(
        self, provided_columns: list
    ) -> Tuple[bool, str]:
    ...
    def check_for_string_in_df(self, df, provided_columns: list):
        invalid_cols = []
        for i in provided_columns:
            inference = infer_dtype(df[i["name"]], skipna=True)
            if inference == "string":
                invalid_cols.append("Invalid col " + i["name"] + " of type string")
            if inference != "string" and self.inference_mapper[inference] != i["type"]:
                invalid_cols.append(
                    "The infered type is different from the one specified , given -> "
                    + i["type"]
                    + " infered -> "
                    + inference
                )
    ...
    def validate_categorical_feature_from_input(
        ...
        for column in categorical_features_map:
            ...
            if not set(input[column].unique()).issubset(
                ...
                category_map_unique_values = set(
                    categorical_features_map[column].keys()
                )
                error_list.append(
...
class BulkLogsValidation(DataSetValidation):
    ...
    def registered_datatype_with_input(
        ...
        for feature_name, feature_dt in registered_dataset.items():
            ...
            match_result = self.registered_datatype_vs_input_datatype(
                feature_dt, input_column_dt
            )
            if not match_result:
    ...
    def prediction_tabular_checks(
        ...
        print(
            " [info] Validating Name & Datatype match for provided columns vs registered."
        )
...
class BulkLogProcessing:
    ...
    def _dataframe_chunks(
        ...
        if len(dataframe) > self.bulk_chunk_size:
            dataframe_chunks = np.array_split(
                dataframe, len(dataframe) / self.bulk_chunk_size
            )
    ...
    def _restructure_bulk_explain_df(
