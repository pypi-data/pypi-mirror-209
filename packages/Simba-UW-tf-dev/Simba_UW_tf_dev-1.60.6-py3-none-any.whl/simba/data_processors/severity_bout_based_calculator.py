...
class SeverityBoutCalculator(ConfigReader, FeatureExtractionMixin):
    ...
    def run(self):
        ...
        self.results = pd.DataFrame(columns=['VIDEO', 'EVENT', 'START TIME', 'END TIME', 'START FRAME', 'END FRAME', 'BOUT TIME', 'SEVERITY'])
    ...
    def visualize(self):
        if self.settings['visualize_event_cnt'] > len(self.results):
        ...
        for idx, r in viz_df.iterrows():
            ...
            clip_path = os.path.join(save_dir, f'{video_name}_{start_frm}_{end_frame}_{severity}.mp4')
