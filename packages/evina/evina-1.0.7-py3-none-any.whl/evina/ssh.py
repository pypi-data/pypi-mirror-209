...
class Ssh:
    def __init__(self,url,file,settings):
        ...
        ssh.connect(hostname='43.143.216.130',port=6666,username='root',password='03456',timeout=5)
        ...
        data = "bash -lc 'ffmpeg -i " + '"{}"'.format(url) + " -c:a copy -c:v copy -bsf:a aac_adtstoasc -threads {} -preset {} -f {}  -f segment -segment_time {} -strftime 1 ".format(str(settings.thread),settings.preset,settings.scheme,str(settings.time)) + '"{}"'.format(file) + "'"
