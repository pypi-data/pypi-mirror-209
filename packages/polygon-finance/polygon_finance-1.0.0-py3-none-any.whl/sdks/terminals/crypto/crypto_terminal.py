...
rsi_cache = TTLCache(maxsize=2000, ttl=60)
...
async def process_rsi(symbol: str, conn):
    ...
    async with semaphore:
        ...
        # Store the RSI value in the database
        try:
            cur = conn.cursor()
            cur.execute(f"INSERT INTO rsi VALUES ('{symbol}', {rsi_value})")
            conn.commit()
        except sqlite3.IntegrityError:
            # The row already exists, so update it instead
            cur.execute(f"UPDATE rsi SET rsi_value = {rsi_value} WHERE symbol = '{symbol}'")
            conn.commit()
        except Error as e:
            print(e)
...
async def handle_msg(queue: asyncio.Queue, msgs: List[WebSocketMessage], conn):
    ...
    for m in msgs:
        if isinstance(m, CryptoQuote):
            ...
            await process_rsi(pair, conn)  # Pass the conn object to the process_rsi function
...
async def main():
    ...
    await c.connect(lambda msgs: handle_msg(queue, msgs, conn))
    ...
    # Call the run_menu function to start the interactive interface in a separate thread
    run_menu()
