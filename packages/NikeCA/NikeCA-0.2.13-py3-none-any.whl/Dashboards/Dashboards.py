...
class Dashboards(Snowflake):
    ...
    def InclusionExclusion(self):
        ...
        return InclusionExclusion(username=self.username, role=self.role, database=self.database,
                                  warehouse=self.warehouse)

    @property
    def IMP(self):
        from IMP.IMP import IMP
        return IMP(username=self.username, role=self.role, database=self.database, warehouse=self.warehouse)
