...
def _decode_duration_ffmpeg(filename: str, index: int, accurate: bool) -> Fraction:
    ...
    ** Extract the duration by the complete decoding of the stream. **
    ...
    >>> _decode_duration_ffmpeg("cutcutcodec/examples/audio.ogg", 0, accurate=True)
    Fraction(16, 1)
...
def _decode_timestamps_ffmpeg(
    ...
    ** Retrieve the exact position of the frames in a stream. **
    ...
    >>> _decode_timestamps_ffmpeg("cutcutcodec/examples/audio.ogg", 0) # doctest: +ELLIPSIS
...
def _estimate_duration_ffmpeg(filename: str, index: int, *, _indirect=True) -> Fraction:
    ...
    ** Extract the duration from the metadata. **
    ...
    >>> _estimate_duration_ffmpeg("cutcutcodec/examples/audio.ogg", 0)
    Fraction(16, 1)
...
def _estimate_len_ffmpeg(filename: str, index: int, *, _indirect=True) -> int:
    ...
    ** Extract the number of frames or samples from the metadata. **
    ...
    >>> _estimate_len_ffmpeg("cutcutcodec/examples/audio.ogg", 0)
    256000
...
def _estimate_rate_ffmpeg(filename: str, index: int, *, _indirect=True) -> Fraction:
    ...
    ** Retrieves via ffmpeg, the metadata concerning the fps or the framerate. **
    ...
    >>> _estimate_rate_ffmpeg("cutcutcodec/examples/audio.ogg", 0)
    Fraction(16000, 1)
...
def get_metadata(
    ...
    ** Call ``ffprobe`` and parse the result as a dictionary. **
    ...
    >>> pprint(get_metadata("cutcutcodec/examples/audio.ogg")) # doctest: +ELLIPSIS
    {'format': {'bit_rate': '26166',
                'duration': '16.000000',
                'filename': 'cutcutcodec/examples/audio.ogg',
...
def get_slices_metadata(
    ...
    ** Get the packets informations for all streams. **
    ...
    >>> headers, data = get_slices_metadata("cutcutcodec/examples/audio.ogg", slice_type="packet")
    >>> pprint(headers)
    [['codec_type',
      'dts',
      'dts_time',
