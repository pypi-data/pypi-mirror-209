...
class main_menu(TUIMenu):
    ...
    class boundary(TUIMenu):
        ...
        class modify(TUIMenu):
            ...
            def __init__(self, path, service):
                self.path = path
                self.service = service
                self.patch_options = self.__class__.patch_options(path + ["patch_options"], service)
                super().__init__(path, service)
    ...
    class preferences(TUIMenu):
        ...
        def __init__(self, path, service):
            ...
            self.prj_app = self.__class__.prj_app(path + ["prj_app"], service)
        ...
        class set(TUIMenu):
                ...
                class by_type(TUIMenu):
                    ...
                    class type_name(TUIMenu):
                        ...
                        def __init__(self, path, service):
                            self.path = path
                            self.service = service
                            self.axis = self.__class__.axis(path + ["axis"], service)
                            self.far_field = self.__class__.far_field(path + ["far_field"], service)
                            self.free_surface = self.__class__.free_surface(path + ["free_surface"], service)
                            self.inlet = self.__class__.inlet(path + ["inlet"], service)
                            self.interface = self.__class__.interface(path + ["interface"], service)
                            self.interior = self.__class__.interior(path + ["interior"], service)
                            self.internal = self.__class__.internal(path + ["internal"], service)
                            self.outlet = self.__class__.outlet(path + ["outlet"], service)
                            self.overset = self.__class__.overset(path + ["overset"], service)
                            self.periodic = self.__class__.periodic(path + ["periodic"], service)
                            self.rans_les_interface = self.__class__.rans_les_interface(path + ["rans_les_interface"], service)
                            self.surface = self.__class__.surface(path + ["surface"], service)
                            self.symmetry = self.__class__.symmetry(path + ["symmetry"], service)
                            self.traction = self.__class__.traction(path + ["traction"], service)
                            self.wall = self.__class__.wall(path + ["wall"], service)
                            super().__init__(path, service)
            ...
            class lights(TUIMenu):
                ...
                def __init__(self, path, service):
                    self.path = path
                    self.service = service
                    self.lighting_interpolation = self.__class__.lighting_interpolation(path + ["lighting_interpolation"], service)
                    super().__init__(path, service)
