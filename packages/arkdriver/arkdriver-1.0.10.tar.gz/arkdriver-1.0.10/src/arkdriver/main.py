...
def assert_ark_api_is_alive(domain: str) -> None:
    response = requests.get(f"{domain}/ping")
...
def main():
    ...
    if response == '1':
        ...
        assert_ark_api_is_alive(URL)
        dialogue(URL)
    elif response == '2':
        assert_nitrado_is_alive()
        assert_ark_api_is_alive(TEST_URL)
        dialogue(TEST_URL)
    elif response == '3':
        assert_nitrado_is_alive()
        assert_ark_api_is_alive(TEST_CLOUD_URL)
