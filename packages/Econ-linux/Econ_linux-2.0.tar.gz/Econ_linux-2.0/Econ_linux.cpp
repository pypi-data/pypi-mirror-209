...
bool initialize()
    {
        ...
        indexList = 0;
        printf(" udev_list_entry_foreach(dev_list_entry, device)");
        udev_list_entry_foreach(dev_list_entry, device)
        {
            ...
            printf(" isValidNode(device_node)" );
            if(isValidNode(device_node))
            {
                ...
                {
                    printf("initialize with TOF_PID and TOF_VID" );
    ...
    bool getDeviceCount(uint32_t *gDeviceCount)
    {
		...
		*gDeviceCount = 0;
        if(!indexList){
            ...
            printf("getDeviceCount - failed NoDeviceConnected");
            return false;
        }
        while (list)
        {
            list &= (list-1);
            *gDeviceCount+=1;
            printf("getDeviceCount - inside the list");
        }
        //return DEPTHVISTAError::setErrno(Result::Ok);
        return true;
	}

    bool getDeviceCount_PY(pybind11::object& gDeviceCount)
    {
        initialize();
        int list=indexList;
        printf("getDeviceCount - inside the list %d\n = ",list);
