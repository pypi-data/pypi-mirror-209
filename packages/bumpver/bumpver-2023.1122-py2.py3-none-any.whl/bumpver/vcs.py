...
VCS_SUBCOMMANDS_BY_NAME = {'git': {'is_usable': 'git rev-parse --git-dir',
    ...
    'git push {remote} --follow-tags {tag} HEAD', 'show_remotes':
    'git config --get remote.origin.url', 'ls_branches': 'git branch -vv'},
    'hg': {'is_usable': 'hg root', 'fetch': 'hg pull', 'ls_tags': 'hg tags',
    'status': 'hg status -umard', 'add_path': 'hg add {path}', 'commit':
    'hg commit --logfile {path}', 'tag': 'hg tag {tag} --message {tag}',
    'push_tag': 'hg push {tag}', 'show_remotes': 'hg paths'}}
