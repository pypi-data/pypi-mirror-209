...
class StatusCaching(threading.Thread):
        ...
        while not self._b_stop.is_set():
            ...
            if self._error_buf and time.time() - last_error_log_ts > self.log_interval:
                ...
                mqtt.publish(
                    ...
                    payload=json.dumps(self._error_buf),
                )
                logger.warning(
            ...
            if time.time() - last_status_log_ts > self.log_interval:
                ...
                mqtt.publish(
                    ...
                    payload=json.dumps(self._status_buf),
                )
                # Write data to redis
                redis.put_data(
                ...
                logger.debug(f"Status message - {json.dumps(self._status_buf)}")
    ...
    def update_status_info(self, info: dict, source_idx: int) -> None:
        ...
        logger.info(f"Updated status info - {self._status_buf}")
...
def get_status_logger(node_id, node_name, node_type):
