...
class STM:
    ...
    # async def _exit_stop(self):
    # print(f"< {self.name} exit: {self.state}")
    ...
    def slowdown(self):
        self.inc_counter('sleep', delta=10, top=90)
...
class DefaultExecutor(ShellSTM):
    def __init__(self, host='localhost', *args, **kw):
        super().__init__(*args, **kw)
        self.host = host
    ...
    async def do_Disconnect(self):
        self.log.info(f"{self.name} connecting to {self.host}!!")
        if self.connection:
            self.log.debug(
        ...
        self.known_hostname = hostname.strip()
        ...
        self.log.warning(f"hostname returned: '{self.known_hostname}'")
