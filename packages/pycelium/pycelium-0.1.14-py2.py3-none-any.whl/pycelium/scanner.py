...
class Settler(Agent):
    ...
    async def do_Idle(self, *args, **kw):
        ...
        if str(current) != str(target):
            ...
            self.s(spec, target)
            path = self.expand("output/{{ host }}.yaml", **self.g('env'))
    ...
    async def _fiber_sentinel(self, *args, **kw):
        ...
        while self.running:
            ...
            if not result:
                self.log.warning(
                    f"No deltas found. Sleeping for a while (60 secs)"
                )
                await self.sleep(60)
                continue
    ...
    # fs
    # --------------------------------------------------
    async def hide_gap_var_fs_any_content(self, delta, *args, **kw):
        """
    ...
    async def hide_gap_var_fs_contains(self, delta, *args, **kw):
        self.log.debug(f"GAP: Resolving: {args}: {kw}")
        await self.sleep()
        foo = 1

    async def hide_gap_var_fs_any_owner(self, delta, *args, **kw):
        """
        ('var', 'fs', '/etc/wireguard/wg0.conf', 'content')
        None
        <content ...>
    ...
    async def hide_gap_var_fs_any_group(self, delta, *args, **kw):
        """
        ('var', 'fs', '/etc/wireguard/wg0.conf', 'content')
        None
        <content ...>
    ...
    async def hide_gap_var_fs_any_mode(self, delta, *args, **kw):
        """
        ('var', 'fs', '/etc/wireguard/wg0.conf', 'content')
        None
        <content ...>
    ...
    async def hide_gap_var_services_any_status_any(self, delta, *args, **kw):
        self.log.debug(f"GAP: Resolving: {delta}")
    ...
    async def hide_gap_var_services_any_restart_any(
        self, delta, *args, **kw
    ):
        """
    ...
    async def hide_gap_var_services_any_watch_any(self, delta, *args, **kw):
        """
        - enable service if it was missing
        - Create .path unit
        - Create restart.service
    ...
    # packages
    # --------------------------------------------------
    async def hide_gap_etc_repository_deb(self, delta, *args, **kw):
        path, old, new = delta
        name = path[-1]
    ...
    async def _gap_var_pkg_pip(self, delta, *args, **kw):
        ...
        status = self.g(installed.prefix, default={}).get(name)
        ...
        if new in ('lastest',):
            if not 'aversion' in status:
                ...
                upgrade = self.new_action(PipListFact, upgradable=True)
        ...
        if need_uninstall:
            install = self.new_action(
                PipPkgInstall, name=name, install=False
            )
            result = await self.wait(install)
            if not any(result):
                self.log.info(f"Package '{name}' uninstalled successfully")
