...
def test_GenerateRavenInput(HYDAT_Path):
    ...
    Path_final_hru_info = os.path.join(
        Routing_Product_Folder, "finalcat_hru_info.shp"
    )  ### River polyline
    ...
    assert filecmp.cmp(
        ...
        os.path.join(Expect_Result_Folder, "RavenInput", "channel_properties.rvp"),
    )
    assert filecmp.cmp(
    ...
    assert filecmp.cmp(
        ...
        os.path.join(Expect_Result_Folder, "RavenInput", "obs", "02LE013_720.rvt"),
    )
    assert filecmp.cmp(
        os.path.join(Output_Folder, "RavenInput", "obs", "02LE015_519.rvt"),
        os.path.join(Expect_Result_Folder, "RavenInput", "obs", "02LE015_519.rvt"),
    )
    assert filecmp.cmp(
        os.path.join(Output_Folder, "RavenInput", "obs", "02LE024_1024.rvt"),
        os.path.join(Expect_Result_Folder, "RavenInput", "obs", "02LE024_1024.rvt"),
    )
    assert filecmp.cmp(
