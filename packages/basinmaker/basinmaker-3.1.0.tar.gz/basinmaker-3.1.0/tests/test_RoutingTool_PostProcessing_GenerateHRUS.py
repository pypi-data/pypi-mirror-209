...
def test_GenerateHRUS():
    ...
    Expect_finalcat_hru_info = Dbf_To_Dataframe(
        os.path.join(Expect_Result_Folder, "finalcat_hru_info.shp")
    ).sort_values(by=["SubId", "HRU_Area"])

    ### transfer resulted product into pandas dataframe
    Result_finalcat_hru_info = Dbf_To_Dataframe(
        os.path.join(Output_Folder, "finalcat_hru_info.shp")
    ).sort_values(by=["SubId", "HRU_Area"])

    print(
        Evaluate_Two_Dataframes(
            Result_finalcat_hru_info, Expect_finalcat_hru_info, Check_Col_NM="HRU_ID"
        )
    )
