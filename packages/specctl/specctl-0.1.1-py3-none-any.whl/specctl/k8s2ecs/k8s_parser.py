...
from k8s2ecs.ingress import k8s_ingress_handler, merge_ingress, create_ingress_target_groups
import logging
...
def associate_svc_to_dep(services, deployments):
    ...
    for svc in services:
        ...
        for dep in deployments:
            dep_namespace = svc.get("deployment_namespace","")
    ...
    for svc in services:
        ...
        for p in ports:
            ...
            if targetPort is None: 
                ...
                continue 
            # if type(targetPort) is int:
            #     service_tags["ecs_service"]=str("%s:%s"%(service_name, str(targetPort)))
            #     continue
            # it is a named port
            dep = svc.get("deployment")
            if dep is None: 
