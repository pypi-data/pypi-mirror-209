...
def encrypt_data() -> None:
    ...
    output_dir = os.path.join(input_dir, "encrypted_data")
    encrypt_GMP(PseudoRandomNumberGenerator(shared_keys[role], base_p), input_dir, output_dir=output_dir)

    print("saving shared key")
    with open(os.path.join(output_dir, "other_shared_key.bin"), "wb") as f:
        f.write(shared_keys[3 - role])
    print("copying over pos.txt")
    shutil.copyfile(f"{input_dir}/pos.txt", os.path.join(output_dir, "pos.txt"))
