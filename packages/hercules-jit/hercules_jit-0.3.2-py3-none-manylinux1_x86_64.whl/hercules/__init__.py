...
def pmap(func, data):
    ...
    if tracing():
        ...
        if not isinstance(func, (pipeline.ops.OpKernel, runtime.object.ObjectBase)):
            raise TypeError(f"hercules.pmap: the first argument '{func}' is not a traceable op")
        if not isinstance(
                data, (pipeline.symbol.BaseSymbol, runtime.List, runtime.Tuple, list, tuple)
        ):
            raise TypeError(f"hercules.pmap: the second argument '{data}' is not supported")
...
def pstarmap(func, data):
    ...
    if tracing():
        ...
        if not isinstance(func, (pipeline.ops.OpKernel, runtime.object.ObjectBase)):
            raise TypeError(f"hercules.pstarmap: the first argument '{func}' is not a traceable op")
        if not isinstance(
                data, (pipeline.symbol.BaseSymbol, runtime.List, runtime.Tuple, list, tuple)
        ):
            raise TypeError(f"hercules.pstarmap: the second argument '{data}' is not supported")
