...
class ScalarNode(ExpressionBaseNode):
    ...
    def to_hercules_ir(self, **kwargs):
...
class IterScalarNode(ScalarNode):
    ...
    def to_hercules_ir(self, key=None, **kwargs):
        if key == "start":
            return self.start.to_hercules_ir()
        if key == "end":
            return self.end.to_hercules_ir()
        if key == "step":
            return self.step.to_hercules_ir()
        return super().to_hercules_ir(**kwargs)
