...
import numpy as np
import hercules
from hercules import pipeline
from hercules.pipeline import _ffi_api, make_op_creator_function
from hercules.pipeline.symbol import Symbol, Variable, Constant, BaseSymbol, make_symbol
from hercules.pipeline._tracing_state import tracing
...
class PytorchModule(object):
    ...
    def __init__(
        ...
        # init hercules cpp resource name and location
        if location is None:
            location = f"./torch_jit_{timestamp}.jit"

        # init hercules cpp pass op and resource
    ...
    def _trans_(self, data):
        ...
        elif isinstance(data, (tuple, hercules.Tuple)):
            raise NotImplementedError("Tuple is not supported as model input")
        elif isinstance(data, (list, hercules.List)):
            return self._trans_list_(data)
        elif isinstance(data, (dict, hercules.Dict)):
            return self._trans_dict_(data)
        elif isinstance(data, (set, hercules.Set)):
            return self._trans_set_(data)
        else:
            if isinstance(data, hercules.array.NDArray):
        ...
        if isinstance(data, list):
            ...
            if isinstance(data, self._torch_Tensor):
                ...
                if self._output_to_cpu:
                    return hercules.array.from_dlpack(
    ...
    def __call__(self, *args):
        with self._torch.no_grad():
            ...
            if "example" not in self._pass_r_options:
                ...
                example = hercules.to_runtime_object(tuple(model_args))
    ...
    def make_pipeline_op(self):
        py_callable = hercules.to_packed_func(self)
