...
from ..env import HERCULES_DEV_MODE
...
from hercules.pipeline.symbol import Symbol, Variable as Var
from hercules.pipeline.symbol import Constant as Const
...
def TraceImpl(func, *args, **kwargs):
    ...
    ----------
    func : callable
        A Python function or a hercules Symbol(s) that will be run with `args`.
...
def trace(func, *args, **kwargs):
    ...
    try:
        ...
        # print(e)
        counter.set('hercules_trace_error_counter', 1)
        if HERCULES_DEV_MODE:
            counter.set('hercules_trace_error', str(e))
            counter.flush()
            raise
        else:
            ex, trans_state = trans_exception(e, use_cc_stacktrace=False)
            counter.set('hercules_trace_error', str(ex))
            counter.flush()
            if trans_state:
...
def load(folder, device):
    """Load a hercules model from folder
...
_HERCULES_GLOBAL_PLUGINS_ = list()


def load_ops(lib_paths):
    """Load extend ops from dynamic lib and register to hercules
    ...
    -------
    """
    global _HERCULES_GLOBAL_PLUGINS_
    ...
    for op_dll in lib_paths:
        ...
        os.chdir(cwd)
        _HERCULES_GLOBAL_PLUGINS_.append(lib)
