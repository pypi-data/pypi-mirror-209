...
from .._ffi.runtime_ctypes import DataType, HerculesDevice, HERCULESArray, HERCULESArrayHandle
from .._ffi.runtime_ctypes import DataTypeCode, hercules_shape_index_t
...
class NDArray(Object):
    ...
    """Lightweight NDArray implementation for hercules runtime
    ...
    Construction method 1
        ...
        Examples:
            >>> import hercules
            >>> nd = hercules.NDArray([1,2,3,4,5,6], [3,2], "int32")
            >>> nd
            [
             [ 1 2 ]
             [ 3 4 ]
    ...
    Construction method 2
        ...
        Examples:
            >>> import hercules
            >>> nd = hercules.NDArray([[1, 2, 3,], [4, 5, 6]], [], "int32")
            >>> nd
            [
             [ 1 2 3 ]
             [ 4 5 6 ]
    ...
    Construction method 3
        ...
        Examples:
            >>> imoprt hercules
            >>> nd = hercules.NDArray([], [2, 3], "float32")
            >>> nd
            [
    ...
    def __init_self__(self):
        tensor_handle = HERCULESArrayHandle()
        handle = self.handle
    ...
    def __setstate__(self, state):
        ...
        handle, code = _ffi.hercules_script_api.steal_object_handle(arr)
        self.handle = handle
        self.type_code = code
        self.__init_self__()
    ...
    def to_list(self):
        """Convert a NDArray to a hercules.List corresponding to the shape
    ...
    def is_contiguous(self):
        ...
        Examples:
            >>> import hercules
            >>> x = hercules.NDArray([1,2,3,4], [2,2], "int32")
            >>> y = x.transpose()
            >>> y.is_contiguous()
            0
    ...
    def __repr__(self):
        res = "<hercules.NDArray shape={0}, {1}>\n".format(
    ...
    def from_numpy(self, source_array):
        """Copy data from a numpy.ndarray to the current NDArray, requiring both to have the same size.
           Note! This method cannot be compiled for use in hercules.script
        ...
        Examples:
            ...
            >>> nd = hercules.NDArray([], arr.shape, str(arr.dtype))
            >>> nd.from_numpy(arr)
            [
...
#            raise TypeError('type %s not supported' % str(type(value)))
    ...
    def release(self):
        _ffi.hercules_script_api.release_object_handle(self)

    def to_dlpack(self):
        return _ffi.hercules_script_api._to_dlpack(self)
...
def add(lhs, rhs):
    ...
    Specific use of the interface: hercules.array.add

    Args:
        lhs (hercules.NDArray or number): Left operand
        rhs (hercules.NDArray or number): Right operand
    ...
    Examples:
        >>> import hercules
        >>> nd1 = hercules.array.rand([2, 3]) # Analogous to numpy.random.rand(2, 3)
        >>> nd1
        [
        ...
        >>> nd3 = hercules.array.add(nd1, nd2)
        >>> nd3
        [
         [
        ...
        [2, 2, 3]
        >>>
        >>> nd3 = hercules.array.add(nd1, 2.0)
        >>> nd3
        [
...
def sub(lhs, rhs):
    ...
    Specific use of the interface: hercules.array.sub
    ...
    Examples:
        ...
        >>> nd3 = hercules.array.sub(nd1, nd2)
        >>> nd3
        [
         [
        ...
        [2, 2, 3]
        >>>
        >>> nd3 = hercules.array.sub(nd1, 2.0)
        >>> nd3
        [
...
def div(lhs, rhs):
    ...
    Specific use of the interface: hercules.array.div
    ...
    Examples:
        ...
        >>> nd3 = hercules.array.div(nd1, nd2)
        >>> nd3
        [
         [
...
def mul(lhs, rhs):
    ...
    Specific use of the interface: hercules.array.mul
    ...
    Examples:
        ...
        >>> nd3 = hercules.array.mul(nd1, nd2)
        >>> nd3
        [
         [
...
def rand(shape):
    ...
    Analogous to: hercules.array.rand
...
def concatenate(seq, axes=0):
    ...
    Specific use of the interface: hercules.array.concatenate

    Args:
        seq (List[hercules.NDArray] or Tuple[hercules.NDArray])
    ...
    Examples:
        >>> import hercules
        >>> nd = hercules.NDArray([0, 1, 2, 3, 4, 5, 6, 7], [2, 2, 2], "int32")
        >>> hercules.array.concatenate([nd, nd], 0)
        [
         [
          [ 0 1 ]
          [ 2 3 ]
        ...
        >>> hercules.array.concatenate([nd, nd], 1)
        [
         [
          [ 0 1 ]
          [ 2 3 ]
        ...
        >>> hercules.array.concatenate([nd, nd], 2)
        [
         [
          [ 0 1 0 1 ]
          [ 2 3 2 3 ]
...
def stack(seq, axes=0):
    ...
    Specific use of the interface: hercules.array.stack
...
#    """Constructs the HERCULES context with the given dev_type dev_id.
#       Note! This method cannot be compiled for use in hercules.script
#
#    Args
...
#    Examples
#        >>> import hercules
#        >>> assert hercules.context("cpu", 1) == hercules.cpu(1)
#        >>> assert hercules.context("gpu", 0) == hercules.gpu(0)
#        >>> assert hercules.context("cuda", 0) == hercules.gpu(0)
#    """
...
#    """Construct the module method of hercules.NDArray
#       Note! This method cannot be compiled for use in hercules.script
#
#    Args:
#        arr (List)
...
def from_numpy(arr, device="cpu"):
    """Construct a module method for hercules.NDArray from numpy.ndarray.
       Note! This method cannot be compiled for use in hercules.script
    ...
    Examples:
        ...
        >>> import hercules
        >>> arr = numpy.random.rand(2, 3)
        >>> nd = hercules.array.from_numpy(arr)
        >>> nd
        [
...
#    """Return a HERCULESArray representation of a numpy array.
#    """
#    data = np_data
#    assert data.flags['C_CONTIGUOUS']
#    arr = HERCULESArray()
#    shape = c_array(hercules_shape_index_t, data.shape)
...
#    -------
#    arr : nd.NDArray
#        The array hercules supported.
#    """
#    shape = c_array(hercules_shape_index_t, shape)
...
def from_dlpack(dltensor):
    ...
    """
    return _ffi.hercules_script_api._from_dlpack(dltensor)
