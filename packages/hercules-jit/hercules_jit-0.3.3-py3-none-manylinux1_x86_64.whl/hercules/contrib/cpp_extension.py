...
def get_link_flags():
    ...
    # -------libturbo.so-------------------#
    if sys.platform.startswith('win32'):
        turbo_soname = "libturbo.dll"
    elif sys.platform.startswith('darwin'):
        turbo_soname = "libturbo.dylib"
    else:
        turbo_soname = "libturbo.so"
    turbopath = find_lib_path(turbo_soname, [libdir, os.path.join(libdir, "turbo/lib")])
    turbo_dir = os.path.dirname(turbopath[0])

    ldflags.append("-L" + turbo_dir)
    ldflags.append("-lturbo")
...
def library_paths():
    ...
    if sys.platform.startswith('win32'):
        ...
        turbo_soname = "libturbo.so"
    turbo_lib_location = find_lib_path(turbo_soname, [lib_dir, os.path.join(lib_dir, "turbo/lib")])
    turbo_dir = os.path.dirname(turbo_lib_location[0])

    if turbo_dir != lib_paths:
        lib_paths.append(turbo_dir)
