...
addLoggingLevel('HERCULES_INFO', logging.INFO - 5)

LIB_PATH = os.environ.get('HERCULES_DSO_DIR', 'dso')

USE_SO_CACHE = os.environ.get('HERCULES_USE_SO_CACHE', '').lower() != 'false'

DISABLE_SCRIPT = os.environ.get('HERCULES_DISABLE_SCRIPT', '').lower() == 'true'
DISABLE_GENERATE_CC = os.environ.get('HERCULES_DISABLE_GENERATE_CC', '').lower() == 'true'
        ...
        if is_function:
            ...
            if bundle_args is None:
                ...
                if not isinstance(bundle_args, (list, set, tuple)):
                    raise TypeError("hercules.script: bundle_args must be None or List")
            ...
            logging.hercules_info('need_bundle: %s', need_bundle)
            try:
...
def toolchain_build(sc_ctx: context.ScriptContext, toolchain: ToolChain):
    ...
    with contrib.util.filelock(base_path):
        ...
        if not hit_cache(so_path):
            logging.hercules_info(
                "hercules compile function/class: [{}:{}]".format(main_node_name, so_path))
...
def build_dso(sc_ctx: context.ScriptContext, use_toolchain=False):
    ...
    with contrib.util.filelock(base_path):
        sopath = base_path + '.so'
        sopath_abi0 = base_path + '_cxx11.so'
        ...
        if not hit_cache(sopath):
            logging.hercules_info("hercules compile function/class: [{}:{}]".format(main_node_name, sopath))
        ...
        sc_ctx.dso_path = (sopath, sopath_abi0)
...
def script(compiling_obj, *, share=False, toolchain=None, bundle_args=None):
    ...
    simple class, compile it to a hercules4 object which mostly
