...
#define HERCULES_NDARRAY_TYPE_SWITCH(dtype, DType, ...)                       \
  switch (::hercules::runtime::DataType::flatten(dtype)) {              \
    ...
    default: {                                                            \
      MXCHECK(false) << ::hercules::runtime::DataType::debug_str(dtype) \
                     << " : unsupported ndarray type";                    \
      break;                                                              \
    }                                                                     \
  }

#define HERCULES_NDARRAY_TYPE_SWITCH_WITH_BOOL(dtype, DType, ...)             \
  switch (::hercules::runtime::DataType::flatten(dtype)) {              \
    case ::hercules::runtime::FlattenedDataType::INT8: {                \
      typedef int8_t DType;                                               \
      { __VA_ARGS__ }                                                     \
