 ...
 public:
  ...
  NativeObjectRegistry& SetConstructor(const FLambda& body) {
    ...
    if (!(std::is_same<typename FLambdaSig::return_type, std::shared_ptr<void>>::value ||
      ...
      MXTHROW << "HERCULES_REGISTER_NATIVE_OBJECT(" << class_name << ") mismatch, expect '"
...
#define HERCULES_NATIVE_OBJECT_VAR_DEF(ClassName) \
  static HERCULES_ATTRIBUTE_UNUSED auto& __make_##HERCULES_NATIVE_OBJECT##ClassName

#define HERCULES_REGISTER_NATIVE_OBJECT(ClassName)                                \
  HERCULES_REGISTER_TYPE_NAME_TRAITS(ClassName);                            \
  HERCULES_STR_CONCAT(HERCULES_NATIVE_OBJECT_VAR_DEF(ClassName), __COUNTER__) = \
