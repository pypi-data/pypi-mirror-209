    ...
    def to_hercules_ir(self, **kwargs):
        if isinstance(self.lhs, NDArrayNode) and is_ndarray_type(self.rhs.kernel_type):
            return self.lhs.buffer.vstore(
                tuple(
                    self.iter_var_names), self.rhs.to_hercules_ir(
...
class AssignScalarNode(StatementBaseNode):
    ...
    def to_hercules_ir(self, **kwargs):
        if isinstance(self.lhs, NDArrayIndexingNode):
            return self.lhs.to_hercules_ir(self.rhs)
        return _ir.AssignStmt(self.lhs.to_hercules_ir(), self.rhs.to_hercules_ir(), self.span)
