...
from hercules import ir as _ir
from hercules.ir.tensor_stmt import decl_buffer
from hercules.kernel.typing import *
from hercules.kernel.typing import NDArrayType as kernelNDArrayT
from .base import *
...
class NDArrayNode(ExpressionBaseNode):
    ...
    def to_hercules_ir(self, iter_var=None, **kwargs):
        if iter_var is None:
...
class NDArrayIndexingNode(ExpressionBaseNode):
    ...
    def to_hercules_ir(self, value=None, **kwargs):
        if value is None:
            return self._read_from()
        return self._write_to(value)

    def _read_from(self):
        return self.ndarray.buffer.vload(tuple(i.to_hercules_ir() for i in self.index))

    def _write_to(self, value):
        value = _ir.PrimCast(self.ndarray.kernel_type.dtype_str(), value.to_hercules_ir())
        return self.ndarray.buffer.vstore(tuple(i.to_hercules_ir() for i in self.index),
                                          value)
    ...
    def buffer_regions(self, rng=None, **kwargs):
        ...
        for idx in self.index:
            start = idx.to_hercules_ir(key="start")
            end = idx.to_hercules_ir(key="end")
            step = idx.to_hercules_ir(key="step")
