...
class Module(object):
    ...
    def __init__(self, handle=None, name=None):
        ...
        self.__holder = sys.modules['hercules']
...
class JITModule(Module):
    ...
    def trace(self, sym):
        ...
        if isinstance(sym, tuple) or isinstance(sym, list):
            ...
            raise RuntimeError(
                f"Type {type(sym)} cannot be traced. Only the values returned by hercules op can be traced. "
            )
...
def LoadModule(folder, name, device):
    warnings.warn("The function hercules.pipeline.LoadModule is deprecated.", DeprecationWarning)
