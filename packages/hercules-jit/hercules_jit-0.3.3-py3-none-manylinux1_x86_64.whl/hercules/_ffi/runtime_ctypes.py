...
hercules_shape_index_t = ctypes.c_int64
...
class HERCULESByteArray(ctypes.Structure):
...
class HerculesDevice(ctypes.Structure):
    ...
    def _GetDeviceAttr(self, device_type, device_id, attr_id):
        ...
        # pylint: disable=import-outside-toplevel
        import hercules.runtime._ffi_api
        return hercules.runtime._ffi_api.GetDeviceAttr(
    ...
    def sync(self):
        ...
        check_call(_LIB.HERCULESSynchronize(self.device_type, self.device_id, None))
...
class HERCULESArray(ctypes.Structure):
    """HERCULESValue in C API"""
    _fields_ = [("data", ctypes.c_void_p),
                ("device", HerculesDevice),
                ("ndim", ctypes.c_int),
                ("dtype", DataType),
                ("shape", ctypes.POINTER(hercules_shape_index_t)),
                ("strides", ctypes.POINTER(hercules_shape_index_t)),
...
HERCULESArrayHandle = ctypes.POINTER(HERCULESArray)
HERCULESTensorHandle = ctypes.POINTER(HERCULESArray)
