...
class AppendCatAsFloat(_tf.keras.layers.Layer):
	...
	def get_config(B):A=super().get_config().copy();A.update({_F:B.vocabulary});return A
	def call(B,x,**F):A=B.lookupLayer(x[_G]);A=_tf.cast(A,x[_A].dtype);A=_tf.expand_dims(_tf.tile(_tf.expand_dims(A,1),[1,_tf.shape(x[_A])[1]]),2);C=_tf.concat([x[_A],A],2);D=_tf.constant([[[[.01]]]]);E=_BM([_MA(x[_B]),_MA(D)]).to_dense();return{_A:C,_B:E,_C:x[_C]}
...
class CatTrainableOffset(_M):
	...
	def compute_output_shape(B,input_shape):A=input_shape;return{_A:(A[_A][0],A[_A][1],B.output_dim),_B:(A[_A][0],A[_A][1],B.output_dim,B.output_dim),_C:(A[_A][0],A[_A][1])}
	def get_config(B):A=super().get_config().copy();A.update({_F:B.vocabulary});return A
	def call(A,x,**F):D=_tf.shape(x[_A])[0];B=A.lookupLayer(x[_G]);B=_tf.expand_dims(_tf.one_hot(B,A.vocabulary_len),2);C=_tf.cond(A.sampling,lambda:_tf.reduce_sum(B*A.bias.sample(D),1),lambda:_tf.reduce_sum(B*A.bias_mean,1));C=_tf.expand_dims(C,1);E=x[_A]+C;return{_A:E,_B:x[_B],_C:x[_C]}