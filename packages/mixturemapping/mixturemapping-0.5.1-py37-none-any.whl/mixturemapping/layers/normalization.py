...
class FixedForward(_FixedBase):
	def call(A,x,**B):x=x.copy();x[_A]=(x[_A]-A.trafo_mean)/A.trafo_std;x[_C]=x[_C]/A.trafo_cov;return x
class FixedBackward(_FixedBase):
	def call(A,x,**B):x=x.copy();x[_A]=x[_A]*A.trafo_std+A.trafo_mean;x[_C]=x[_C]*A.trafo_cov;return x