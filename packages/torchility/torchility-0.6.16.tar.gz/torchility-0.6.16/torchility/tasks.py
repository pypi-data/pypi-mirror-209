    ...
    def do_metric(self, preds, targets, stage, batch_size, dl_idx):  # 指标计算
        ...
        for metric in metrics:
            ...
            if isinstance(value, dict):  # 一个函数中计算多个指标，返回一个字典
                # value_dict = {f"{stage}_{dict_metric_name(metric.name, k)}_step":v for k, v in value.items()}
                for k, v in value.items():
                    metric_values[dict_metric_name(metric.name, k)] = v.item()
                # self.log_dict(value_dict, prog_bar=metric.on_bar, on_step=metric.on_step, on_epoch=metric.on_epoch, batch_size=batch_size)
            else:
                metric_values[f"{metric.name}"] = value.item()
                # self.log(f"{stage}_{metric.name}_step", value, prog_bar=metric.on_bar, on_step=metric.on_step, on_epoch=metric.on_epoch, batch_size=batch_size)
        return metric_values
