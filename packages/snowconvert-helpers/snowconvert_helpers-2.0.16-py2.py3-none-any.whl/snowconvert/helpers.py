...
class BeginLoading:
    ...
    def import_file_to_table(target_schema_table, define_columns, define_file, begin_loading_columns, begin_loading_values, field_delimiter = ",", skip_header=0, input_data_place_holder=None, con=None):
        ...
        if define_columns != "" or begin_loading_values != []:
            ...
            selectValues = BeginLoading._get_select_values(column_names, begin_loading_values, unordered_actual_values)
            exec(f"COPY INTO {target_schema_table} {begin_loading_columns} FROM (SELECT {selectValues} FROM {stage_name})  FILE_FORMAT=(TYPE=CSV SKIP_HEADER={skip_header} {fileFormat}) ON_ERROR=CONTINUE", con=con)
        else:
            exec(f"COPY INTO {target_schema_table} FROM {stage_name}  FILE_FORMAT=(TYPE=CSV SKIP_HEADER={skip_header} {fileFormat}) ON_ERROR=CONTINUE", con=con)        
