...
class WebElem:
    ...
    def click(self, timeout=10000):
        logger.info("click")
        try:
            self.find_element().click(timeout=timeout)
        except:
            self._driver.screenshot("loc_fail")
            raise NoSuchElementException(f"[elem loc fail]")

    def input(self, text, timeout=10000):
        logger.info("input")
        try:
            self.find_element().fill(text, timeout=timeout)
        except:
            self._driver.screenshot("loc_fail")
            raise NoSuchElementException(f"[elem loc fail]")
    ...
    def select(self, option: str = None, text: str = None):
        logger.info("select")
        try:
            if option:
                self.find_element().select_option(option)
            if text:
                self.find_element().select_text(text)
        except:
            self._driver.screenshot("loc_fail")
            raise NoSuchElementException(f"[elem loc fail]")
