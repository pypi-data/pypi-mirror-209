...
class WebElem:
    ...
    def select(self, option: str = None, text: str = None, timeout=10):
        logger.info("select")
        try:
            if option:
                self.find_element().select_option(option, timeout=timeout*1000)
            if text:
                self.find_element().select_text(text, timeout=timeout*1000)
        except:
            self._driver.screenshot("loc_fail")
            raise NoSuchElementException(f"[elem loc fail]")
