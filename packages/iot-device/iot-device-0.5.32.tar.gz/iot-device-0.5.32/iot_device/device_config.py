...
class _Resource:
    ...
    @property
    def includes(self):
        inc = self._param.get('include-patterns', self._dev._spec.get('include-patterns', [ './**/*.py', './**/*.mpy', './**/' ]))
        if isinstance(inc, str): inc = [ inc ]
        return inc
    
    @property
    def excludes(self):
        exc = self._param.get('exclude-patterns', self._dev._spec.get('exclude-patterns', [ 'boot_out.txt' ]))
    ...
    def files(self):
        ...
        result = []
        includes_ = self.includes
        excludes_ = self.excludes
        path = os.path.join(self.path, self.name)
        if os.path.isfile(Env.expand_path(path)): return [ self.name ]
        try:
            with cd(path):
                for inc in includes_:
