...
def create_odb_as_json(arguments):
    ...
    ddds = load_ddds(odb_as_json["ddd_names"], arguments.overridden_ddd_config_paths, raw_config["rerank_amount"])
...
class OrchestratedDomainBundle(AsJSONMixin):
    def __init__(self, argument_dict):
        ...
        self._rerank_amount = argument_dict["rerank_amount"]
        ...
        for key in self._confidence_thresholds:
            self._confidence_thresholds[key] = self._confidence_thresholds[key]
        for key in self._confidence_prediction_thresholds:
            self._confidence_prediction_thresholds[key] = self._confidence_prediction_thresholds[key]
