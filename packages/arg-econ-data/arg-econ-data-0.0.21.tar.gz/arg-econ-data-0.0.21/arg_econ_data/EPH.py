def download_ephs(
    ...
    """
        Descarga las bases de la EPH en formato csv y las guarda en path.
        Args:
    @path (str): Ruta donde se descargan los archivos.
    @year (int, list, tuple, optional): Año o rango de años [desde, hasta] a descargar. False equivale a todos los años.
    @sleep (int, optional): Tiempo de espera entre descargas (en segundos). Defaults to 30.
    """
    import time
    ...
    for año, onda in list(zip(ephs.año, ephs.trimestre_u_onda)):
        try:
            get_microdata(
                year=año, trimester_or_wave=onda, type=type, download=True, path=path
            )
        except INDECError:
            print(
        ...
        print(
            f"Esperando {sleep} segundos para no saturar al servidor...",
            flush=True,
            sep="",
            end="\r",
        )
        time.sleep(sleep)  # Para no saturar al servidor
...
def get_microdata(
    ...
    if (
        year < 2019
    ):  # FIXME: humai bloquea mucho mas el acceso, el download termina crasheando. Habria que probar pasarlo a las bases de indec
        ...
        if download:
            ...
            df = pd.read_csv(url, engine="python", encoding="unicode_escape")
    elif year >= 2019:
