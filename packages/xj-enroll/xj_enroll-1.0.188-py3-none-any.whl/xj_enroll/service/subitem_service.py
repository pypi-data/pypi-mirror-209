...
class SubitemService:
    ...
    def edit(params, subitem_id=None):
        # 参数验证
        subitem_id, is_pass = force_transform_type(variable=subitem_id, var_type="int", default=0)
        ...
        if enroll_id and not enroll_id == subitem_info.get("enroll_id"):
            return None, "报名ID为(" + str(enroll_id) + ")的报名不存在ID为(" + str(subitem_id) + ")的分项"
        ...
        params = format_params_handle(
            param_dict=params,
            filter_filed_list=[i.name for i in EnrollSubitem._meta.fields]
        )
        # 开始修改
        try:
