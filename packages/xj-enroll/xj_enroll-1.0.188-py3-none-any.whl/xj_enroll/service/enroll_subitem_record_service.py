...
class EnrollSubitemRecordService:
    ...
    def edit(params=None, pk=None):
        pk, is_pass = force_transform_type(variable=params.pop("id", None) or pk, var_type="int", default=0)
        enroll_id, is_pass = force_transform_type(variable=params.pop("enroll_id", None), var_type="int")  # 流程控制得到报名ID,报名分项不允许更改报名ID。
        # 数据合法型验证
        query_obj = EnrollSubitemRecord.objects.annotate(enroll_id=F("enroll_subitem__enroll_id")).filter(id=pk).values("id", "enroll_id")
        if not query_obj:
        ...
        params = format_params_handle(
            param_dict=params,
            filter_filed_list=[
                "price", "count", "subitem_amount",
                "enroll_subitem_status_code", "reply", "remark", "files", "photos"
            ]
        )
        try:
            instance = EnrollSubitemRecord.objects.filter(id=pk).update(**params)
        except Exception as e:
