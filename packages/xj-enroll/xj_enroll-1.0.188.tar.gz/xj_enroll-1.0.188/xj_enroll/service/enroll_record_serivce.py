...
class EnrollRecordServices:
    ...
    def record_list(params: dict, need_pagination: bool = True, exclude_code=124, **kwargs):
        ...
        record_obj = EnrollRecord.objects.order_by(sort).annotate(thread_id=F("enroll__thread_id")).filter(**params)
    ...
    def record_edit(params, pk):
        ...
        user_id, is_pass = force_transform_type(variable=params.pop("user_id", None), var_type="int")  # 流报名分项不允许更改用户ID。
        if not pk:
        ...
        if enroll_id and not enroll_id == record_info.get("enroll_id"):
            return None, "该报名(" + str(enroll_id) + ")不存在ID为(" + str(pk) + ")的记录"
