...
import sys, os, re, argparse, argcomplete, json, datetime, io, xml, xmltodict, collections
...
def xlsx2dict(input, verbose=False, formulas=False):
	js = {'workbook': {'sheet': []}}
	...
	for s in wb:
		...
		for ri,r in enumerate(s.rows):
			...
			row = {'@number': '%d' % ri, '@letter': '%d' % (ri + 1), 'col': []}
...
def main():
	...
	:
		js = xlsx2dict(input, verbose=args.verbose, formulas=args.formula)
