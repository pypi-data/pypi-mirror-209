...
def get_professionalroles_id_vs_name(self, postgres_creds=None, save_to_postgres=False):
    ...
    df = pd.DataFrame({'id':ids, 'name':names}).drop_duplicates(subset=['id'])
...
def send_recommended_vacancies_to_chatid(self, chat_id):
    ...
    if not is_subscription_valid and not is_notified:
        ...
        self.bot.send_message(chat_id, 'Срок вашей подписки подошел к концу. Чтобы продолжить использование бота нажмите <b>"Оплатить подписку"</b> и пополните баланс:', reply_markup=keyboard, parse_mode="HTML")
