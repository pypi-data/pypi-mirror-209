...
class Api:
    ...
    def _call_api(
        ...
        try:
            ...
            if response.status_code != requests.codes["no_content"]:
                ...
                elif response.headers.get("content-type") and response.headers["content-type"].strip().startswith(
                    "application/json"
                ):
    ...
    def simulation_stop(cls, model_uuid: str, simulation_uuid: str) -> dict:
        api = f"{Api.BASE_PATH}/models/{model_uuid}/simulations/{simulation_uuid}/events"
    ...
    @staticmethod
    def _get_result_urls(batch_resp: str, names: T.List[str]) -> T.List[str]:
        errors = []
        urls = []
