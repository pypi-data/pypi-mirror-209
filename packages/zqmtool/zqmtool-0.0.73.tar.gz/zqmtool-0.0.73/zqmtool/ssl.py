...
# util functions
def representation_checking(x):
    assert np.all([len(e) > 0 for e in x])
    assert np.amin(x) >= 0 and np.amax(x) <= 1
...
def update_beta(method, beta, loss_infomin=None, acc=None, min_val=None, set_beta=None):
    ...
    if method.lower() == 'InfoGoal'.lower():
        ...
        return add_beta(beta) if acc > 0.90 else reduce_beta(beta)
