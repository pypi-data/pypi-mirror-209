...
import ubotlibs
from ubotlibs import __license__, __version__, enums, raw, utils
from ubotlibs.crypto import aes
from ubotlibs.errors import (AuthBytesInvalid, BadRequest, CDNFileHashMismatch,
                             ChannelPrivate, ListenerCanceled,
                             SessionPasswordNeeded, VolumeLocNotFound)
from ubotlibs.handlers.handler import Handler
from ubotlibs.methods import Methods
from ubotlibs.session import Auth, Session
from ubotlibs.storage import FileStorage, MemoryStorage
from ubotlibs.types import TermsOfService, User
from ubotlibs.utils import ginput
...
class Client(Methods):
    ...
    Parameters:
        ...
        in_memory (``bool``, *optional*):
            ...
            :meth:`~ubotlibs.Client.export_session_string` before stopping the client to get a session string you can
            ...
            try:
                if not self.phone_number:
                    while True:
                        ...
                        confirm = (
                            await ginput(f'Is "{value}" correct? (y/N): ')
                        ).lower()
    ...
    def set_parse_mode(self, parse_mode: Optional["enums.ParseMode"]):
        ...
        Parameters:
            parse_mode (:obj:`~ubotlibs.enums.ParseMode`):
            ...
            if not await self.storage.api_id():
                if self.api_id:
                    ...
                    while True:
                        try:
                            value = int(
                                await ginput("Enter the api_id part of the API key: ")
                            )
            ...
            if not include:
                ...
                for path, handlers in include:
                    ...
                    try:
                        ...
                        log.warning(
                            f'[{self.name}] [LOAD] Ignoring non-existent module "{module_path}"'
                        )
                        continue

                    if "__path__" in dir(module):
                        log.warning(
                            f'[{self.name}] [LOAD] Ignoring namespace "{module_path}"'
                        )
                        continue
            ...
            if exclude:
                for path, handlers in exclude:
                    ...
                    try:
                        ...
                        log.warning(
                            f'[{self.name}] [UNLOAD] Ignoring non-existent module "{module_path}"'
                        )
                        continue

                    if "__path__" in dir(module):
                        log.warning(
                            f'[{self.name}] [UNLOAD] Ignoring namespace "{module_path}"'
                        )
                        continue
        ...
        try:
            ...
            if isinstance(r, raw.types.upload.File):
                ...
                try:
                    ...
                    raise e
        except ubotlibs.StopTransmission:
            raise
        except Exception as e:
            log.error(e, exc_info=True)
