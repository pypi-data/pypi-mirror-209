...
import ubotlibs
from ubotlibs import utils
from ubotlibs.handlers import (CallbackQueryHandler, ChatJoinRequestHandler,
...
from ubotlibs.raw.types import (UpdateBotCallbackQuery,
...
class Dispatcher:
    ...
    def __init__(self, client: "ubotlibs.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return (
                await ubotlibs.types.Message._parse(
                    self.client,
                    update.message,
                    users,
                    chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await ubotlibs.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler,
            )

        async def user_status_parser(update, users, chats):
            return (
                ubotlibs.types.User._parse_user_status(self.client, update),
                UserStatusHandler,
            )

        async def inline_query_parser(update, users, chats):
            return (
                ubotlibs.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler,
            )

        async def poll_parser(update, users, chats):
            return (ubotlibs.types.Poll._parse_update(self.client, update), PollHandler)

        async def chosen_inline_result_parser(update, users, chats):
            return (
                ubotlibs.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler,
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                ubotlibs.types.ChatMemberUpdated._parse(
        ...
        async def chat_join_request_parser(update, users, chats):
            return (
                ubotlibs.types.ChatJoinRequest._parse(
                    self.client, update, users, chats
                ),
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except ubotlibs.StopPropagation:
                                raise
                            except ubotlibs.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.error(e, exc_info=True)
