...
async def compose(clients: List["ubotlibs.Client"], sequential: bool = False):
    """Run multiple clients at once.

    This method can be used to run multiple clients at once and can be found directly in the ``pyrogram`` package.

    If you want to run a single client, you can use Client's bound method :meth:`~ubotlibs.Client.run`.

    Parameters:
        clients (List of :obj:`~ubotlibs.Client`):
    ...
    Example:
        .. code-block:: python
            ...
            import asyncio
            from ubotlibs import Client, compose
