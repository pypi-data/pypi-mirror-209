...
from ubotlibs.types import (CallbackQuery, InlineKeyboardMarkup, InlineQuery,
                            Message, ReplyKeyboardMarkup, Update)


class Filter:
    async def __call__(self, client: "ubotlibs.Client", update: Update):
...
def create(func: Callable, name: str = None, **kwargs) -> Filter:
    ...
    Parameters:
        func (``Callable``):
            ...
            The *client* argument refers to the :obj:`~ubotlibs.Client` that received the update.
            The *update* argument type will vary depending on which `Handler <handlers>`_ is coming from.
            For example, in a :obj:`~ubotlibs.handlers.MessageHandler` the *update* argument will be a :obj:`~ubotlibs.types.Message`; in a :obj:`~ubotlibs.handlers.CallbackQueryHandler` the *update* will be a :obj:`~ubotlibs.types.CallbackQuery`.
        ...
        **kwargs (``any``, *optional*):
            ...
            :meth:`~ubotlibs.filters.command` or :meth:`~ubotlibs.filters.regex`.
    """
    return type(
...
"""Filter messages that contain :obj:`~ubotlibs.types.Audio` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Document` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Photo` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Sticker` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Animation` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Game` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Video` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Voice` note objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.VideoNote` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Contact` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Location` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Venue` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Poll` objects."""
...
"""Filter messages that contain :obj:`~ubotlibs.types.Dice` objects."""
...
def command(
    ...
    Parameters:
        commands (``str`` | ``list``):
            ...
            field of the :obj:`~ubotlibs.types.Message`.
    ...
    async def func(flt, client: ubotlibs.Client, message: Message):
...
def regex(pattern: Union[str, Pattern], flags: int = 0):
    ...
    - :obj:`~ubotlibs.types.Message`: The filter will match ``text`` or ``caption``.
    - :obj:`~ubotlibs.types.CallbackQuery`: The filter will match ``data``.
    - :obj:`~ubotlibs.types.InlineQuery`: The filter will match ``query``.
