...
from ubotlibs.raw.core import TLObject
...
class RPCError(Exception):
    ...
    def raise_it(rpc_error: "raw.types.RpcError", rpc_type: Type[TLObject]):
        ...
        if error_id not in exceptions[error_code]:
            raise getattr(
                import_module("ubotlibs.errors"), exceptions[error_code]["_"]
            )(
                value=f"[{error_code} {error_message}]",
                rpc_name=rpc_name,
                is_unknown=True,
        ...
        raise getattr(
            import_module("ubotlibs.errors"), exceptions[error_code][error_id]
