...
from ubotlibs import enums, raw
...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~ubotlibs.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~ubotlibs.Client.sign_in` or :meth:`~ubotlibs.Client.sign_up`).

        next_type (:obj:`~ubotlibs.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~ubotlibs.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~ubotlibs.Client.resend_code`.
    """
