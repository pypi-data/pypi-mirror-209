...
/** Forward declarations of functions used in SystemCPtolemyInterface_PT_SC_UpSample */
extern "C" 
 { 
...
int sc_main(int argc, char* argv[]) {
	 ...
	 SystemCPtolemyInterface_PT_SC_UpSample ptolemyInterface("PtolemyInterface", adstoken);
...
SystemCPtolemyInterface_PT_SC_UpSample::SystemCPtolemyInterface_PT_SC_UpSample(sc_module_name name, char * connectiontoken) : sc_module(name) { 
	 int err = 0;
	 interfacelist = newSCADS_InterfaceList();
	 setSCADS_Connection( connectiontoken, &connection, 1, 1 , 3);


/* Adding input ports in the interface list to check for compatibility with ADS */
	 if (insertSCADS_Interface(interfacelist, "input", "INPUT", "DOUBLE")) err = 1;
...
/* Adding user defined variables in the interface list to check for compatibility with ADS */
	 if (insertSCADS_Interface(interfacelist, "Factor", "PARAM", "INT")) err = 1;
	 if (insertSCADS_Interface(interfacelist, "Phase", "PARAM", "INT")) err = 1;
	 if (insertSCADS_Interface(interfacelist, "Fill", "PARAM", "DOUBLE")) err = 1;
...
/* Initializing user defined variables. */
	 Factor = getSCADS_InterfaceByName("Factor", connection, "PARAM" );
	 Phase = getSCADS_InterfaceByName("Phase", connection, "PARAM" );
	 Fill = getSCADS_InterfaceByName("Fill", connection, "PARAM" );
...
SystemCPtolemyInterface_PT_SC_UpSample::~SystemCPtolemyInterface_PT_SC_UpSample(){
...
int SystemCPtolemyInterface_PT_SC_UpSample::getIntegerParamValue(SCADS_Interface * param, int index) { 
	  return (int) getSCADS_ParamValue(param, index, connection);
 } 

/* ************************************************************/

double SystemCPtolemyInterface_PT_SC_UpSample::getDoubleParamValue(SCADS_Interface * param, int index) {
	 return (double) getSCADS_ParamValue(param, index, connection);
 } 

/* ************************************************************/

int SystemCPtolemyInterface_PT_SC_UpSample::getParamArraySize(SCADS_Interface * param) { 
	 return getSCADS_PortNumTokens(param);
 }

/* ************************************************************/

void SystemCPtolemyInterface_PT_SC_UpSample::go() {
	 int token;
	 while (1) { 
	...
	/* Reading inputs from ADS. These are interface outputs. */
		 for( token=0; token < getSCADS_PortNumTokens(pin_input); token++) {
			 input.write( (double) getSCADS_PortValue(pin_input, token) );
 		 }
