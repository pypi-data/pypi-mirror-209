# BD Radius/Nuclues Test:
# rfsource.lock_awg  # This prevents signal from being loaded into awg, but will lead to confusion.
...
class TeledyneT3AWGxxxxModulatedDiff(base.NoRFSourceModulated):
    firmware_map = {}
    display_order = ["on", "initialized", "ch", "port", "freq", "a_p",
    ...
    def __new__(cls, error_model, port, config_filename="",
                ...
                lock_awg=False,
                switch_id="USB::0x20CE::0x0022"):
        self = super(TeledyneT3AWGxxxxModulatedDiff, cls).__new__(cls, error_model, port, config_filename)
        # Define object ATTRIBUTES
        self.resource_id = resource_id
        self.lock_awg = lock_awg
        self.switch_id = switch_id
        self._ch = 0
        return self

    def __getnewargs__(self):
        return tuple(list(super(TeledyneT3AWGxxxxModulatedDiff, self).__getnewargs__())
                     + [self.resource_id, self.lock_awg, self.switch_id])
    ...
    def __getstate__(self, state={}):
        state = super(TeledyneT3AWGxxxxModulatedDiff, self).__getstate__(state=state)
        state["_ch"] = self._ch
        return state
    ...
    def __info__(self):
        super(TeledyneT3AWGxxxxModulatedDiff, self).__info__()
        rtol = 1/2**16
        v_atol = 1e-3
        ...
        self.info["lock_awg"] = Info("lock_awg", read=True, write=True, check=True)
    ...
    def connect_handles(self):
        ...
        super(TeledyneT3AWGxxxxModulatedDiff, self).connect_handles()

    def preset(self):
        super(TeledyneT3AWGxxxxModulatedDiff, self).preset()
    ...
    def _a_p(self, _a_p):
        if not self.lock_awg:
            ...
            iq = interp1d(np.linspace(0, 1, Settings().t_points), iq, kind="linear")(np.linspace(0, 1, t_points))  # Upsample or Downsample
            ...
            TeledyneT3AWGxxxx.download_waveform_bug(self, b'MMEM:DOWN:DATA #' + prefix + n_bytes + q_bytes)
...
class TeledyneT3AWGxxxxModulatedIQ(base.NoRFSourceModulated):
    firmware_map = {}
    ...
    def __new__(cls, error_model, port, config_filename="",
                resource_id='TCPIP0::172.16.0.40::inst0::INSTR'):
        self = super(TeledyneT3AWGxxxxModulatedIQ, cls).__new__(cls, error_model, port, config_filename)
        # Define object ATTRIBUTES
        self.resource_id = resource_id
        return self

    def __getnewargs__(self):
        return tuple(list(super(TeledyneT3AWGxxxxModulatedIQ, self).__getnewargs__()) + [self.resource_id])
    ...
    def __getstate__(self, state={}):
        state = super(TeledyneT3AWGxxxxModulatedIQ, self).__getstate__(state=state)
        return state
    ...
    def __info__(self):
        super(TeledyneT3AWGxxxxModulatedIQ, self).__info__()
        rtol = 1/2**16
        v_atol = 1e-3
        a_atol = v_atol/np.sqrt(50.)
        v_max = 2*(self._config["voltage_limit"] - v_atol)
    ...
    def connect_handles(self):
        rm = visa.ResourceManager(os.getenv('VISA_LIB', '@py'))
        self.handles["awg"] = rm.open_resource(self.resource_id)
        super(TeledyneT3AWGxxxxModulatedIQ, self).connect_handles()

    def preset(self):
        super(TeledyneT3AWGxxxxModulatedIQ, self).preset()
        if self.unique_handle(self.handles['awg']):
            TeledyneT3AWGxxxx.preset(self)
        ...
        TeledyneT3AWGxxxx.download_waveform_bug(self, b'MMEM:DOWN:DATA #' + prefix + n_bytes + bytes)
        self.handles["awg"].query("*OPC?")
