...
/*!
    ...
    \brief A Private d_ptr of QtGroupBoxPropertyBrowser.

    \sa QtGroupBoxPropertyBrowser
*/
class QtGroupBoxPropertyBrowserPrivate
{
...
public:
    ...
    struct WidgetItem
    {
        ...
        QComboBox *pkAvg{nullptr};
        QComboBox *format{nullptr};
        QtDoubleEdit *minimum{nullptr};
        QtDoubleEdit *maximum{nullptr};
        QtBoolEdit *check{nullptr};
...
unsigned short QtGroupBoxPropertyBrowserPrivate::numColumns() const
{
    unsigned short num_cols = 2;
    unsigned short attr_index = 0;

    while (attr_index < AttributeNameMap.size() - 1) {  // Ignore BrowserCol::NONE
        if (m_attributes & 1 << attr_index)
            num_cols++;
        attr_index++;
    }
    return num_cols;
...
BrowserCol QtGroupBoxPropertyBrowserPrivate::columnToAttribute(const int col) const
{
    unsigned short col_index = col - 2;  // Ignore {name, value} columns
    unsigned short attr_index = 0;
    BrowserCol attr = BrowserCol::NONE;

    if (col > numColumns() - 1)
        return attr;

    while (col_index <= 0 && attr_index < AttributeNameMap.size() - 1) {  // Ignore BrowserCol::NONE
        if (m_attributes & 1 << attr_index) {
            attr = BrowserCol(1 << attr_index);
            col_index--;
        }
        attr_index++;
    }
...
void QtGroupBoxPropertyBrowserPrivate::propertyInserted(QtBrowserItem *index, QtBrowserItem *afterIndex)
{
    ...
    if (!parentItem) {
        ...
        if (!parentItem->groupBox) {
            ...
            if (parentItem->widget) {
                ...
                unsigned short attr_index = 0;
                if (m_attributes & BrowserCol::UNIT) {
                    if (parentItem->unit) {
                        l->removeWidget(parentItem->unit);
                        parentItem->unit->setParent(parentItem->groupBox);
                        parentItem->layout->addWidget(parentItem->unit, 0, attr_index+ 2, 1, 1);
                        attr_index++;
                    }
                } if (m_attributes & BrowserCol::PKAVG) {
                    if (parentItem->pkAvg){
                        l->removeWidget(parentItem->pkAvg);
                        parentItem->pkAvg->setParent(parentItem->groupBox);
                        parentItem->layout->addWidget(parentItem->pkAvg, 0, attr_index+ 2, 1, 1);
                        attr_index++;
                    }
                } if (m_attributes & BrowserCol::FORMAT) {
                    if (parentItem->format){
                        l->removeWidget(parentItem->format);
                        parentItem->format->setParent(parentItem->groupBox);
                        parentItem->layout->addWidget(parentItem->format, 0, attr_index+ 2, 1, 1);
                        attr_index++;
                    }
                } if (m_attributes & BrowserCol::CHECK) {
                    if (parentItem->check){
                        l->removeWidget(parentItem->check);
                        parentItem->check->setParent(parentItem->groupBox);
                        parentItem->layout->addWidget(parentItem->check, 0, attr_index+ 2, 1, 1, Qt::AlignRight);
                        attr_index++;
                    }
                }
            ...
            if (parentItem->line) {
                ...
                parentItem->layout->addWidget(parentItem->line, 1, 0, 1, numColumns());
            }
            l->addWidget(parentItem->groupBox, oldRow, 0, 1, numColumns());
    ...
    if (m_attributes & BrowserCol::MINIMUM) {
        newItem->minimum = dynamic_cast<QtDoubleEdit *>(createAttributeEditor(index->property(), parentWidget,
                                                                              BrowserCol::MINIMUM));
        if (newItem->minimum)
            layout->addWidget(newItem->minimum, row, ind + 2, 1, 1);
        ind++;
    } if (m_attributes & BrowserCol::MAXIMUM) {
        newItem->maximum = dynamic_cast<QtDoubleEdit *>(createAttributeEditor(index->property(), parentWidget,
                                                                              BrowserCol::MAXIMUM));
        if (newItem->maximum)
            layout->addWidget(newItem->maximum, row, ind + 2, 1, 1);
        ind++;
    } if (m_attributes & BrowserCol::UNIT) {
        newItem->unit = dynamic_cast<QComboBox *>(createAttributeEditor(index->property(), parentWidget,
                                                                        BrowserCol::UNIT));
        if (newItem->unit)
            layout->addWidget(newItem->unit, row, ind + 2, 1, 1);
        ind++;
    } if (m_attributes & BrowserCol::PKAVG) {
        newItem->pkAvg = dynamic_cast<QComboBox *>(createAttributeEditor(index->property(), parentWidget,
                                                                         BrowserCol::PKAVG));
        if (newItem->pkAvg)
            layout->addWidget(newItem->pkAvg, row, ind + 2, 1, 1);
        ind++;
    } if (m_attributes & BrowserCol::FORMAT) {
        newItem->format = dynamic_cast<QComboBox *>(createAttributeEditor(index->property(), parentWidget,
                                                                          BrowserCol::FORMAT));
        if (newItem->format)
            layout->addWidget(newItem->format, row, ind + 2, 1, 1);
        ind++;
    } if (m_attributes & BrowserCol::CHECK) {
        newItem->check = dynamic_cast<QtBoolEdit *>(createAttributeEditor(index->property(), parentWidget,
                                                                          BrowserCol::CHECK));
        if (newItem->check)
...
void QtGroupBoxPropertyBrowserPrivate::updateItem(WidgetItem *item)
{
    ...
    if (item->label) {
        ...
        palette.setColor(QPalette::WindowText, property->foreground().color());
    ...
    if (item->widget) {
        ...
        if (label){
            ...
            item->widget->setMaximumWidth(metrics.boundingRect("12.34<-180.00").width());
        }
    }
    if (item->unit){
        ...
        item->unit->setMaximumWidth(40+metrics.boundingRect("dBmrW").width());
    }
    if (item->pkAvg){
        ...
        item->pkAvg->setMaximumWidth(40+metrics.boundingRect("avg").width());
    }
    if (item->format){
        ...
        item->format->setMaximumWidth(45+metrics.boundingRect("Log<Deg").width());
    }
    if (item->minimum){
        ...
        item->minimum->setMaximumWidth(metrics.boundingRect("-12.3e10").width());
    }
    if (item->maximum){
        ...
        item->maximum->setMaximumWidth(metrics.boundingRect("+12.3e10").width());
    }
    if (item->check){
...
/*!
    ...
    \image html qtgroupboxpropertybrowser.png "Qt Group Property Browser"
...
BrowserCol QtGroupBoxPropertyBrowser::attributes() const
{
    return d_ptr->m_attributes;
}

void QtGroupBoxPropertyBrowser::setAttributes(BrowserCol attributes)
{
    if (d_ptr->m_attributes == attributes)
        return;

    d_ptr->m_attributes = attributes;
