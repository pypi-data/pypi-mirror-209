...
/*!
    ...
    \brief A Private d_ptr of QtTreePropertyBrowser.

    \sa QtTreePropertyBrowser
*/
class QtTreePropertyBrowserPrivate
{
...
void QtPropertyEditorView::mousePressEvent(QMouseEvent *event)
{
    ...
    if (item) {
        if ((item != m_editorPrivate->editedItem()) && (event->button() == Qt::LeftButton)
                && (header()->logicalIndexAt(event->position().toPoint().x()) > 0)
...
public:
    QtPropertyEditorDelegate(QObject *parent = 0)
        : QItemDelegate(parent), m_editorPrivate(0), m_editedItem(0), m_editedWidget(0), m_editedColumn(-1)
        {}
...
void QtTreePropertyBrowserPrivate::init(QWidget *parent)
{
    ...
    m_attributes = BrowserCol::NONE;
    q_ptr->setAttributes(BrowserCol::NONE);
...
void QtTreePropertyBrowserPrivate::updateAttributes()
{
    ...
    if (m_treeWidget->columnCount() > 1){
        ...
        attributeWidth = metrics.boundingRect("12.34<-180.00").width();
    ...
    if (m_attributes & BrowserCol::MINIMUM) {
        ...
        m_treeWidget->header()->setSectionResizeMode(2 + index, QHeaderView::Interactive);
        attributeWidth = metrics.boundingRect("-12.3e10").width();
        ...
        labels.append(QCoreApplication::translate("QtTreePropertyBrowser",
                                                  attributeNames.at(index).toUtf8()));
        m_treeWidget->header()->setSectionResizeMode(2 + index, QHeaderView::Interactive);
        attributeWidth = metrics.boundingRect("dBmrW______").width();
        m_treeWidget->header()->resizeSection(2 + index, attributeWidth);
        index++;
        ...
        labels.append(QCoreApplication::translate("QtTreePropertyBrowser",
                                                  attributeNames.at(index).toUtf8()));
        m_treeWidget->header()->setSectionResizeMode(2 + index, QHeaderView::Interactive);
        attributeWidth = metrics.boundingRect("avg______").width();
        m_treeWidget->header()->resizeSection(2 + index, attributeWidth);
        index++;
        ...
        labels.append(QCoreApplication::translate("QtTreePropertyBrowser",
                                                  attributeNames.at(index).toUtf8()));
        m_treeWidget->header()->setSectionResizeMode(2 + index, QHeaderView::Interactive);
        attributeWidth = metrics.boundingRect("Log<Deg______").width();
        m_treeWidget->header()->resizeSection(2 + index, attributeWidth);
        index++;
        ...
        labels.append(QCoreApplication::translate("QtTreePropertyBrowser",
                                                  attributeNames.at(index).toUtf8()));
        m_treeWidget->header()->setSectionResizeMode(2 + index, QHeaderView::Interactive);
        attributeWidth = metrics.boundingRect("?").width();
        m_treeWidget->header()->resizeSection(2 + index, 0);
    }
    m_treeWidget->setHeaderLabels(labels);
}
...
unsigned short QtTreePropertyBrowserPrivate::numColumns() const
{
...
/*!
    ...
    \image html qttreepropertybrowser.png "Qt Tree Property Browser"
...
BrowserCol QtTreePropertyBrowser::attributes() const
{
    return d_ptr->m_attributes;
}

void QtTreePropertyBrowser::setAttributes(BrowserCol attributes)
{
