...
if sys.platform == "win32": from win32api import GetCursorPos, SetCursorPos
if sys.platform == "win32": from win32api import mouse_event, keybd_event
if sys.platform == "win32": import win32con
if sys.platform == "win32": from win32clipboard import OpenClipboard, GetClipboardData, SetClipboardText, EmptyClipboard, CloseClipboard
...
class ButtonPresser(object):
    ...
    @staticmethod
    def mouse_left_click(x, y, delay):
        SetCursorPos((x, y))
    ...
    @staticmethod
    def mouse_right_click(x, y, delay):
        SetCursorPos((x, y))
        mouse_event(win32con.MOUSEEVENTF_RIGHTDOWN, x, y, 0, 0)
        mouse_event(win32con.MOUSEEVENTF_RIGHTUP, x, y, 0, 0)
        time.sleep(delay)

    @staticmethod
    def mouse_drag(x_start, y_start, delay, x_stop, y_stop, delay2):
        SetCursorPos((x_start, y_start))
        mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, x_start, y_start, 0, 0)
        time.sleep(delay)
        SetCursorPos((x_stop, y_stop))
        mouse_event(win32con.MOUSEEVENTF_LEFTUP, x_stop, y_stop, 0, 0)
        time.sleep(delay2)

    @staticmethod
    def key_press(name, delay=0.05):
        keys = name.split("+")
        modifier = getattr(win32con, "VK_%s" % ((keys[0].upper()),)) if len(keys) > 1 else 0
        try:
            key_code = ord(keys[-1])
        except TypeError:
            key_code = getattr(win32con, "VK_%s" % ((keys[-1].upper()),))
        # Send Message
        if modifier:
            keybd_event(modifier, 0, 1 | 0, 0)
        keybd_event(key_code, 0, 1 | 0, 0)
        time.sleep(delay)
        keybd_event(key_code, 0, 1 | win32con.KEYEVENTF_KEYUP, 0)
        if modifier:
            keybd_event(modifier, 0, 1 | win32con.KEYEVENTF_KEYUP, 0)
...
class MousePositionEdit(QLineEdit):
    cursorMove = QtCore.Signal(object)

    def __init__(self, *args):
        super(MousePositionEdit, self).__init__(*args)
    ...
    def handleCursorMove(self, pos):
        self.setText("(%d, %d)" % (pos.x(), pos.y()))


class KeySequenceEdit(QLineEdit):
    def __init__(self, *args, keySequence=QKeySequence("")):
...
if __name__ == "__main__":
    ...
    time.sleep(2)
    presser.key_press("CONTROL+J")
    time.sleep(2)
    presser.key_press("MENU+V")
    time.sleep(2)
    presser.key_press("MENU+A")
    time.sleep(2)
    presser.key_press("MENU+C")
    time.sleep(2)
    presser.key_press("MENU+V")
    time.sleep(2)
    presser.key_press("MENU+A")
    time.sleep(2)
    presser.key_press("MENU+X")
    time.sleep(2)
    presser.key_press("MENU+V")
    # while True:
    #     time.sleep(0.1)
    #     pos = presser.get_mouse_position()
    #     print(pos)
