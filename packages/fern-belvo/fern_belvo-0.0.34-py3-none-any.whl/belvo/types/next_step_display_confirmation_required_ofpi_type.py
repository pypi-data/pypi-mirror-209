...
class NextStepDisplayConfirmationRequiredOfpiType(str, enum.Enum):
    ...
    def visit(
        ...
        if self is NextStepDisplayConfirmationRequiredOfpiType.OPEN_FINANCE_DISPLAY_PAYMENT_METHOD_INFORMATION:
            return open_finance_display_payment_method_information()
        if self is NextStepDisplayConfirmationRequiredOfpiType.OPEN_FINANCE_DISPLAY_CONFIRMATION_REQUIRED:
            return open_finance_display_confirmation_required()
        if self is NextStepDisplayConfirmationRequiredOfpiType.OPEN_FINANCE_DISPLAY_NEEDS_REDIRECT:
            return open_finance_display_needs_redirect()
        if self is NextStepDisplayConfirmationRequiredOfpiType.OPEN_FINANCE_DISPLAY_PAYMENT_PROCESSING:
            return open_finance_display_payment_processing()
        if self is NextStepDisplayConfirmationRequiredOfpiType.OPEN_FINANCE_DISPLAY_PAYMENT_SUCCEEDED:
            return open_finance_display_payment_succeeded()
        if self is NextStepDisplayConfirmationRequiredOfpiType.OPEN_FINANCE_DISPLAY_PAYMENT_FAILED:
