...
class NextStepDisplayCredentialsRequiredPseType(str, enum.Enum):
    ...
    def visit(
        ...
        if self is NextStepDisplayCredentialsRequiredPseType.PSE_DISPLAY_PAYMENT_METHOD_INFORMATION:
            return pse_display_payment_method_information()
        if self is NextStepDisplayCredentialsRequiredPseType.PSE_DISPLAY_CREDENTIALS_REQUIRED:
            return pse_display_credentials_required()
        if self is NextStepDisplayCredentialsRequiredPseType.PSE_DISPLAY_NEEDS_REDIRECT:
            return pse_display_needs_redirect()
        if self is NextStepDisplayCredentialsRequiredPseType.PSE_DISPLAY_TOKEN_REQUIRED:
            return pse_display_token_required()
        if self is NextStepDisplayCredentialsRequiredPseType.PSE_DISPLAY_CUSTOMER_BANK_ACCOUNTS:
            return pse_display_customer_bank_accounts()
        if self is NextStepDisplayCredentialsRequiredPseType.PSE_DISPLAY_CONFIRMATION_REQUIRED:
            return pse_display_confirmation_required()
        if self is NextStepDisplayCredentialsRequiredPseType.PSE_DISPLAY_PAYMENT_PROCESSING:
            return pse_display_payment_processing()
        if self is NextStepDisplayCredentialsRequiredPseType.PSE_DISPLAY_PAYMENT_FAILED:
            return pse_display_payment_failed()
        if self is NextStepDisplayCredentialsRequiredPseType.PSE_DISPLAY_PAYMENT_SUCCEEDED:
