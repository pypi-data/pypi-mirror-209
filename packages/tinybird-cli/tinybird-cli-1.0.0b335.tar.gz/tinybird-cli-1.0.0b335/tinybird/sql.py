...
def wrap_finalize_aggregation(sql: str, describe_result, fm_group=None):
    if not fm_group:
...
def col_name(name: str, backquotes=True) -> str:
    """
    >>> col_name('`test`', True)
    '`test`'
    >>> col_name('`test`', False)
...
def mark_error_string(s: str, i: int, line=1) -> str:
    """
...
def format_parse_error(table_structure, i: int, position: int, hint, line=0):
    message = f"{hint}\n" if hint else ""
...
def parse_table_structure(schema: str) -> List[Dict]:  # noqa: C901
...
def _parse_table_structure(schema: str):  # noqa: C901
    ...
    def lookahead_matches(lookup: Iterable):
        s = schema[i:]
...
def engine_can_be_replicated(engine: str) -> bool:
    """
...
def engine_supports_delete(engine: str) -> bool:
    """
