...
class LatentClassMixedModel(MixedLogit):
    ...
    def fit(self, X, y, varnames=None, alts=None, isvars=None, num_classes=2,
        ...
        if mxl_init:
            mxl = MixedLogit()
            mxl_varnames = np.unique(np.concatenate(class_params_spec))
            X_mxl_idx = [ii for ii, name in enumerate(varnames) if name in mxl_varnames]
            if hasattr(X, 'values'):  # i.e. pandas dataframe
                X_mxl = X.values[:, X_mxl_idx]
            else:
                X_mxl = X[:, X_mxl_idx]
            ...
            for i in range(num_classes):
                init_class_betas[i] = mxl.coeff_
            # add some small noise - otherwise can't distinguish classes
            init_class_betas = init_class_betas + 0.1 * np.random.normal(0, 1, len(init_class_betas))
