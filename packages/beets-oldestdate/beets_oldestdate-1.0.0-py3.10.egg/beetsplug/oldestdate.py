...
class OldestDatePlugin(BeetsPlugin):
    ...
    def _import_task_choice(self, task, session):
        ...
        while not self._has_work_id(recording_id):
            ...
            recording_year_string = None if recording_date is None else str(recording_date['year']) + str(
                recording_date['month']) + str(recording_date['day'])
        ...
        for recording_field in ('year', 'month', 'day'):
            if recording_field in oldest_date.keys():
                ...
                if self.config['overwrite_year'] and recording_field == 'year':
                    ...
                    item['year'] = oldest_date['year']
                    new_date = oldest_date['month'] + oldest_date['day']
                    self._log.warning('Overwriting date field for: {0.artist} - {0.title} from {1} to {2}', item,
                                      item['date'], new_date)
                    item['date'] = new_date
                write = True
