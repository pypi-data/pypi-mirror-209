...
from launch.jobs import urwid_color
...
drain_button = urwid.Button('Drain Job')
stop_button = urwid.Button('Stop Job')
error_widget = urwid.Text('')


def get_main_frame(job_id: int):
    header_widget = urwid.Pile([
        urwid.AttrMap(urwid.Text(f'Job ID: {job_id}', align='center'),
                      'header', 'bold'),
        urwid.Divider('-'),
        job_info_widget,
        urwid.Divider(),
    ...
    footer_widget = urwid.Pile([
        urwid.Divider(),
        urwid.AttrMap(
            urwid.Text('Job Operations', align='center'),
            'header',
            'bold',
        ),
        urwid.Divider('-'),
...
def update_job_info(data):
    try:
        json_data = json.loads(data)
        status_data.set_text(str(json_data['job_status']))
...
async def get_job_info(file_descriptor, job_id, server_address):
    ...
    try:
        ...
        error_widget.set_text(f'Get job info error: {str(e)}')


async def get_logs(job_id, server_address):
    creds = cache.get_user_creds(server_address)
    ws_endpoint = server_address.replace('http://', 'ws://').replace(
        'https://', 'wss://')
    if ws_endpoint.endswith('/'):
    ...
    try:
        status_text, _ = status_data.get_text()
        # Don't start fetching logs until the job is deployed.
        while status_text == 'N/A' or status_text == 'DEPLOYING':
        ...
        error_widget.set_text(f'Get logs error: {str(e)}')
