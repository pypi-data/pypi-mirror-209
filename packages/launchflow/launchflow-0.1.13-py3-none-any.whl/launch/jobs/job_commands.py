...
from launch.jobs import local_utils, jobs_helper, remote_utils
...
STREAM_JOB_LOGS_HELP_TEXT = 'Whether or not to stream the job logs after submission.'  # noqa: E501
STREAM_TO_EXTENSION_HELP_TEXT = 'Whether or not to stream the job logs to the extension.'  # noqa: E501
...
def submit(
    ...
    pipeline_id: Optional[str] = constants.PIPELINE_OPTION,
    ...
    stream_job_logs: bool = typer.Option(default=False,
                                         help=STREAM_JOB_LOGS_HELP_TEXT),
    ...
    if not local:
        ...
        if all([account_id is None, project_id is None, pipeline_id is None]):
            typer.echo(
                'You must specify an account, project, or pipeline to run '
                'this job remotely.')
            raise typer.Exit(1)
    ...
    if local:
        ...
        job_id = local_utils.submit_job_to_ray_cluster(
    ...
    if not local:
        ...
        if working_dir or requirements_file:
            try:
                ...
                job_id = remote_utils.send_deploy_request(
    ...
    if stream_job_logs:
        ...
        if local:
            async_tasks.append(
                loop.create_task(local_utils.stream_job_logs_async(job_id)))
        else:
            async_tasks.append(
                loop.create_task(
                    remote_utils.stream_job_logs(job_id,
    ...
    if stream_to_extension:
        ...
        if local:
            async_tasks.append(
                loop.create_task(
                    local_utils.stream_job_info_async(
                        job_id, extension_server_address)))
        else:
            async_tasks.append(
                loop.create_task(
                    remote_utils.stream_job_info(job_id,
                                                 launchflow_server_address,
                                                 bearer_token,
                                                 extension_server_address)))
...
@app.command(help='Stream info from a running job.')
def stream(
    job_id: str = typer.Argument(..., help='The job id to stream info for.'),
    local: bool = constants.LOCAL_OPTION,
    extension_server_address: str = constants.EXTENSION_SERVER_ADDRESS_OPTION,
    launchflow_server_address: str = constants.
    LAUNCHFLOW_SERVER_ADDRESS_OPTION,
    ...
    if local:
        ...
        async_tasks.append(
            loop.create_task(
                local_utils.stream_job_info_async(job_id,
                                                  extension_server_address)))
    else:
        async_tasks.append(
            loop.create_task(
                remote_utils.stream_job_logs(job_id, launchflow_server_address,
                                             bearer_token,
                                             extension_server_address)))
        async_tasks.append(
            loop.create_task(
                remote_utils.stream_job_info(job_id, launchflow_server_address,
                                             bearer_token,
                                             extension_server_address)))
...
@app.command(help='Ping info for a list of jobs.')
def ping(
    job_ids: List[str] = typer.Argument(...,
                                        help='The job ids to fetch info for.'),
    local: bool = constants.LOCAL_OPTION,
    extension_server_address: str = constants.EXTENSION_SERVER_ADDRESS_OPTION,
    launchflow_server_address: str = constants.
    LAUNCHFLOW_SERVER_ADDRESS_OPTION,
    ...
    if local:
        local_utils.ping_job_info(job_ids, extension_server_address)
    else:
        raise NotImplementedError('Pinging remote jobs is not yet supported.')


@app.command(help='Stop a running job.')
def stop(
    job_id: str = typer.Argument(..., help='The job id to stop.'),
    local: bool = constants.LOCAL_OPTION,
    launchflow_server_address: str = constants.
    LAUNCHFLOW_SERVER_ADDRESS_OPTION,
    bearer_token: str = constants.BEARER_TOKEN_OPTION,
    ...
    if local:
        local_utils.stop_job(job_id)
    else:
        remote_utils.stop_job_cli(job_id, launchflow_server_address,
                                  bearer_token)


@app.command(help='Drain a running job.')
def drain(
    job_id: str = typer.Argument(..., help='The job id to drain.'),
    local: bool = constants.LOCAL_OPTION,
    launchflow_server_address: str = constants.
    LAUNCHFLOW_SERVER_ADDRESS_OPTION,
    bearer_token: str = constants.BEARER_TOKEN_OPTION,
    ...
    if local:
        local_utils.drain_job(job_id)
    else:
        remote_utils.drain_job_cli(job_id, launchflow_server_address,
                                   bearer_token)


@app.command(help='Attach to a job.')
def attach(
    job_id: str = typer.Argument(..., help='The job id to attach to.'),
    local: bool = constants.LOCAL_OPTION,
    launchflow_server_address: str = constants.LAUNCHFLOW_SERVER_ADDRESS_OPTION
):
    if local:
        raise NotImplementedError(
            'Attaching to local jobs is not yet supported.')
    else:
        aiorun(remote_utils.run_console_ui(job_id, launchflow_server_address))


@app.command(help='List jobs for a pipeline.')
def list(
    pipeline_id: Optional[str] = typer.Option(
        ..., help='The pipeline to list jobs for.'),
    running: bool = typer.Option(
        False,
        '--running',
        '-r',
        help='If --running is set only running pipeline will be returned.'),
    launchflow_server_address: str = constants.
    LAUNCHFLOW_SERVER_ADDRESS_OPTION,
):
    creds = cache.get_user_creds(launchflow_server_address)
    response = jobs_helper.list_jobs(
        pipeline_id=pipeline_id,
        running=running,
        creds=creds,
        launchflow_server_address=launchflow_server_address)

    utils.print_response('Jobs', response)


@app.command(help='Get logs for a job.')
def get_logs(
    job_id: str = typer.Argument(..., help='The job id to attach to.'),
    launchflow_server_address: str = constants.LAUNCHFLOW_SERVER_ADDRESS_OPTION
):
    remote_utils.get_logs(job_id, launchflow_server_address)
