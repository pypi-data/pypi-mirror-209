...
X, y = shaperone.datasets.boston()
model = xgboost.XGBRegressor().fit(X, y)

# explain the model's predictions using SHAP
# (same syntax works for LightGBM, CatBoost, scikit-learn, transformers, Spark, etc.)
explainer = shaperone.Explainer(model)
shap_values = explainer(X)

# visualize the first prediction's explanation
shaperone.plots.waterfall(shap_values[0])
```
...
shaperone.plots.text(shap_values[0, :, "POSITIVE"])
```
...
e = shaperone.DeepExplainer(model, background)
# ...or pass tensors directly
# e = shaperone.DeepExplainer((model.layers[0].input, model.layers[-1].output), background)
shap_values = e.shap_values(x_test[1:5])

# plot the feature attributions
shaperone.image_plot(shap_values, -x_test[1:5])
```
...
X, y = shaperone.datasets.imagenet50()
to_explain = X[[39, 41]]

# load the ImageNet class names
url = "https://s3.amazonaws.com/deep-learning-models/image-models/imagenet_class_index.json"
fname = shaperone.datasets.cache(url)
...
shaperone.image_plot(shap_values, to_explain, index_names)
```
...
X_train, X_test, Y_train, Y_test = train_test_split(*shaperone.datasets.iris(), test_size=0.2, random_state=0)
svm = sklearn.svm.SVC(kernel='rbf', probability=True)
svm.fit(X_train, Y_train)

# use Kernel SHAP to explain test set predictions
explainer = shaperone.KernelExplainer(svm.predict_proba, X_train, link="logit")
shap_values = explainer.shap_values(X_test, nsamples=100)

# plot the SHAP values for the Setosa output of the first instance
shaperone.force_plot(explainer.expected_value[0], shap_values[0][0, :], X_test.iloc[0, :], link="logit")
```
<p align="center">
