...
def lambda_handler(event, context):
    try:
        ...
        crude_oil_prod: 'CrudeOilProduction' = CrudeOilProduction()
        crude_oil_prod.get_crude_oil_production()
        raw_data: Dict = crude_oil_prod.get_all_productions

        result: Dict = to_sql(raw_data=raw_data, category="productions")
        for sql_file in result.get("file_name"):
            client.upload_file(Filename=sql_file,
                               Bucket="gasprice-dataset",
                               Key="dataset/crude_oil/productions/%s" % (os.path.basename(sql_file)))
