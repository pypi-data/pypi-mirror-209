...
def setup_cache(context: CertoraContext) -> None:
    ...
    if not context.disable_auto_cache_key_gen and os.environ.get("CERTORA_DISABLE_AUTO_CACHE") is None:
...
"""
dually-defined arguments are command line arguments that can also be passed as a setting.
...
def __check_bool_arg_and_explicit_setting_consistency(context: CertoraContext, arg_name: str,
    ...
    3. The setting appears multiple times with conflicting truth values, like --settings -ci_mode=false,-ci_mode=true
...
def check_arg_and_setting_consistency(context: CertoraContext) -> None:
    ...
    At the end of this function, all the dually-defined argument values will appear in both the argument namespace and
