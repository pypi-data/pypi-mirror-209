...
@LastEditTime: 2023-05-22 18:29:00
...
class TaskConsoleModel():
    ...
    def __get_task_gear_count(act_id, module_id, task_type, complete_type, user_id, task_gear_count_model):
        """
            ...
            try:
                ...
                try:
                    ...
                    if not task_gear_count:
                        ...
                        task_gear_count_model.update_table("now_count=now_count+%s,remark=remark+%s", "id=%s", params=[task_queue_dict["now_count"], ',' + task_queue_dict["remark"], task_gear_count.id])
                    if task_info_dict:
                        now_count = self.__get_task_gear_count() + task_queue_dict["now_count"]
                        #判断子任务是否完成
