...
@LastEditTime: 2023-05-22 14:04:15
...
class StatConsoleModel():
    ...
    def console_task_queue(self):
        ...
        """
        for i in range(10):
            j = threading.Thread(target=self._process_threshold_task_queue, args=[i])
            j.start()

    def _process_threshold_task_queue(self, mod_value):
        """
        ...
        while True:
            try:
                ...
                try:
                    ...
                    task_gear_report_model = TaskGearReportModel(db_transaction=db_transaction)
                    ...
                    task_gear_report = task_gear_report_model.get_dict("act_id=%s and task_type=%s and user_id=%s and create_day=%s and module_id=%s", params=[task_queue_dict["act_id"], task_queue_dict["task_type"], task_queue_dict["user_id"], create_day, task_queue_dict["module_id"]])

                    db_transaction.begin_transaction()
                    if not task_gear_report:
                        task_gear_report = TaskGearReport()
                        task_gear_report.app_id = task_queue_dict["app_id"]
                        task_gear_report.act_id = task_queue_dict["act_id"]
                        task_gear_report.module_id = task_queue_dict["module_id"]
                        task_gear_report.user_id = task_queue_dict["user_id"]
                        task_gear_report.open_id = task_queue_dict["open_id"]
                        task_gear_report.task_type = task_queue_dict["task_type"]
                        task_gear_report.now_count = task_queue_dict["now_count"]
                        task_gear_report.create_day = create_day
                        task_gear_report.create_month = create_month
                        task_gear_report.create_date = SevenHelper.get_now_datetime()
                        task_gear_report_model.add_entity(task_gear_report)
                    else:
                        task_gear_report.now_count = task_gear_report.now_count + 1
                        task_gear_report_model.update_table("now_count=now_count+%s", "id=%s", params=[task_queue_dict["now_count"]])
                    if task_info_dict:
                        #判断子任务是否完成
                        ...
                        if config_json and config_json.__contains__("gear_list"):
                            ...
                            for sub_config_json in config_json:
                                task_count_id_md5 = CryptoHelper.md5_encrypt_int(f"{task_queue_dict['act_id']}_{task_queue_dict['module_id']}_{task_queue_dict['task_type']}_{sub_config_json['id']}_{sub_config_json['user_id']}")
                                if decimal.Decimal(task_gear_report.now_count) > decimal.Decimal(sub_config_json["gear"]):
