        ...
        while True:
            try:
                ...
                try:
                    ...
                    task_gear_count = task_gear_count_model.get_dict("act_id=%s and task_type=%s and user_id=%s and create_day=%s and module_id=%s", params=[task_queue_dict["act_id"], task_queue_dict["task_type"], task_queue_dict["user_id"], create_day, task_queue_dict["module_id"]])
                    ...
                    if not task_gear_count:
                        ...
                        task_gear_count.now_count = task_gear_count.now_count + 1
                        task_gear_count_model.update_table("now_count=now_count+%s", "id=%s", params=[task_queue_dict["now_count"]])
                    if task_info_dict:
                        #判断子任务是否完成
                        ...
                        if config_json and config_json.__contains__("gear_list"):
                            ...
                            for sub_config_json in config_json:
                                ...
                                if decimal.Decimal(task_gear_count.now_count) > decimal.Decimal(sub_config_json["gear"]):
