...
@LastEditTime: 2023-05-22 14:03:39
...
from seven_cloudapp_frame.models.db_models.task.task_gear_report_model import *
...
class TaskBaseModel(FrameBaseModel):
    ...
    def add_gear_task_accumulate(self, app_id, act_id, module_id, user_id, open_id, task_type, value):
        """
        ...
        :last_editors: HuangJianYi
        """
        task_gear_report = TaskGearReport()
        task_gear_report.app_id = app_id
        task_gear_report.act_id = act_id
        task_gear_report.module_id = module_id
        task_gear_report.user_id = user_id
        task_gear_report.open_id = open_id
        task_gear_report.task_type = task_type
        task_gear_report.now_count = value
        task_gear_report.create_day = SevenHelper.get_now_day_int()
        task_gear_report.create_month = SevenHelper.get_now_month_int()
        task_gear_report.create_date = SevenHelper.get_now_datetime()

        redis_init = SevenHelper.redis_init(config_dict=self.redis_config_dict)
        redis_init.rpush(f"task_gear_list:{str(user_id % 10)}", SevenHelper.json_dumps(task_gear_report))

    def convert_gear_task_result(self, act_id, module_id, user_id, task_info_dict, config_json, task_count_list):
        """
        ...
        result["get_ways"] = 1 # 领取方式(0-直接领取 1-通过领取接口领取)

        task_gear_report_model = TaskGearReportModel(context=self.context)
        condition_where = ConditionWhere()
        condition_where.add_condition("act_id=%s and taks_type=%s and user_id=%s")
        params = [act_id, task_info_dict["task_type"], user_id]
        ...
        if module_id:
            ...
            params.append(module_id)
        task_gear_report_dict = task_gear_report_model.get_dict(condition_where.to_string(),params=params)
        result["current"] = task_gear_report_dict["now_count"] if task_gear_report_dict else 0  # 当前总累计值
        ...
        for sub_config_json in config_json:
            ...
            sub_result["reward_name"] = "" #奖励名称
            sub_result["reward_value"] = 1 #奖励数量
    ...
    def get_client_task_list_v2(self, app_id, act_id, module_id, user_id, task_types="", daily_repeat_browse=False):
        ...
        for task_info_dict in task_info_list:
            ...
            result["reward_value"] = 0 # 奖励值
            result["completed_ids"] = [] # 完成的ids，比如：已收藏的商品ids或者已完成的子任务ids
            result["get_ways"] = 0 # 领取方式(0-直接领取 1-通过领取接口领取)
            ...
            #购买指定商品
                ...
                result_list.append(result)
            #有子任务列表的相关累计任务
            elif config_json.__contains__("threshold_list") == True:
                result_list.append(self.convert_threshold_task_result(act_id,module_id,user_id,task_info_dict,config_json,task_count_list))
            ...
            if invoke_result_data.success == True:
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_sub_type:
                        ...
                        elif config_json.__contains__("sub_list") == True: # 每个子任务使用总的累加计数
                            sub_config_json = [sub_config_json for sub_config_json in config_json["sub_list"] if sub_config_json["id"] == task_sub_type]
            ...
            if invoke_result_data.success == True:
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if is_limit == True:
                        ...
                        self.add_task_log(task_type=task_type, task_sub_type='', app_id=app_id, act_id=act_id, module_id=module_id, user_id=user_info_dict["user_id"], open_id=user_info_dict["open_id"], user_nick=user_info_dict["user_nick"],task_name=task_info_dict["task_name"],log_title=task_info_dict["task_name"],reward_type=1, reward_object_id=reward_object_id, reward_name="", reward_num=reward_value, handler_name=handler_name, request_code=self.request_code, info_json=info_json)
                else:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = task_invoke_result_data.error_code
                    invoke_result_data.error_message = task_invoke_result_data.error_message
            ...
            if invoke_result_data.success == True:
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_info_dict["complete_type"] == 1 and task_count.modify_day == now_day:
                        ...
                        invoke_result_data.data["reward_value"] = reward_value
                        self.add_task_stat(task_type,app_id, act_id, module_id, user_info_dict["user_id"], user_info_dict["open_id"], reward_value, is_stat)
                        self.add_task_log(task_type=task_type, task_sub_type='', app_id=app_id, act_id=act_id, module_id=module_id, user_id=user_info_dict["user_id"], open_id=user_info_dict["open_id"], user_nick=user_info_dict["user_nick"],task_name=task_info_dict["task_name"],log_title=log_title,reward_type=1, reward_object_id=reward_object_id, reward_name="", reward_num=reward_value, handler_name=handler_name, request_code=self.request_code, info_json=info_json)
                else:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = task_invoke_result_data.error_code
                    invoke_result_data.error_message = task_invoke_result_data.error_message
            ...
            if invoke_result_data.success == True:
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_count.modify_day == now_day:
                        ...
                        invoke_result_data.data["reward_value"] = reward_value
                        self.add_task_stat(task_type,app_id, act_id, module_id, user_info_dict["user_id"], user_info_dict["open_id"], reward_value, is_stat)
                        self.add_task_log(task_type=task_type, task_sub_type='', app_id=app_id, act_id=act_id, module_id=module_id, user_id=user_info_dict["user_id"], open_id=user_info_dict["open_id"], user_nick=user_info_dict["user_nick"],task_name=task_info_dict["task_name"],log_title=f"签到{task_count.complete_count}天",reward_type=1, reward_object_id=reward_object_id, reward_name="", reward_num=reward_value, handler_name=handler_name, request_code=self.request_code, info_json=info_json)
                else:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = task_invoke_result_data.error_code
                    invoke_result_data.error_message = task_invoke_result_data.error_message
