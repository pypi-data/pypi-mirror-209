...
class ParapheurImportTemplate(object):
    ...
    def launch(self):
        ...
        bar = IncrementalBar('Importing %s ...' % self.name, max=len(list_to_handle),
...
class UsersImport(ParapheurImportTemplate):
    ...
    def __each_func__(self, element):
        encoded_username = element['username']
        # 1 - Create users
...
class GroupsImport(ParapheurImportTemplate):
    ...
    def __each_func__(self, element):
        ...
        for member in element['users']:
            ...
            client.dopost("/parapheur/groupes/%s/%s" % (element['id'], member['shortName']))
        return element
...
class BureauxAdvancedImport(ParapheurImportTemplate):
    ...
    def __after_func__(self):
        for i, user in enumerate(self.users):
            if os.path.isfile("users/admins/%s.json" % user['username']):
                with open("users/admins/%s.json" % user['username'], "r") as json_file:
                    # Remove duplicates
...
class TyposImports(ParapheurImportTemplate):
    ...
    def __each_func__(self, element):
        # Create type
        client.dopost("/parapheur/types/%s" % element['id'], element)

        # Eventually do pades thing
        if os.path.isfile("types/%s_pades.json" % element['id']):
            with open("types/%s_pades.json" % element['id']) as data_file:
                pades_info = json.load(data_file)
                client.doput("/parapheur/types/%s/overridePades" % element['id'],
                             pades_info['overridedTdt'])
        # Eventually do actes thing
        if os.path.isfile("types/%s_actes.json" % element['id']):
            with open("types/%s_actes.json" % element['id']) as data_file:
                actes_info = json.load(data_file)['overridedTdt']
                if os.path.isfile("advanced/certs/%s" % actes_info['name']):
                    base64cert = client.load_binary_file("advanced/certs/%s" % actes_info['name'])
                    actes_info['cert'] = base64cert
                    client.doput("/parapheur/types/%s/overrideActes" % element['id'],
                                 actes_info)
        # Eventually do helios thing
        if os.path.isfile("types/%s_helios.json" % element['id']):
            with open("types/%s_helios.json" % element['id']) as data_file:
                helios_info = json.load(data_file)['overridedTdt']
                if os.path.isfile("advanced/certs/%s" % helios_info['name']):
                    base64cert = client.load_binary_file("advanced/certs/%s" % helios_info['name'])
                    helios_info['cert'] = base64cert
                    client.doput("/parapheur/types/%s/overrideHelios" % element['id'],
                                 helios_info)

        # Create all subtypes
        for subtype in element['sousTypes']:
            with open("types/%s/%s.json" % (element['id'], subtype['id'])) as data_file:
            ...
            # Create subtype
            client.dopost(
                "/parapheur/types/%s/%s" % (element['id'], subtype['id']), subtype_info)
        return element
                ...
                for com in elements:
                    ...
                    if "nomImage" in com:
                        binary_img = client.load_binary_file(
                            "calques/%s/%s" % (old_id, com['nomImage']))
...
class AdvancedImport(ParapheurImportTemplate):
    ...
    def __each_func__(self, element):
        with open("advanced/%s.json" % element) as data_file:
            ...
            if os.path.isfile("advanced/certs/%s" % data['name']):
                base64cert = client.load_binary_file("advanced/certs/%s" % data['name'])
