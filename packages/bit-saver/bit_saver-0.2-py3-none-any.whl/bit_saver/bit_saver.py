...
class BitSaver:
    ...
    def save_single_ticker_data(self, ticker, start_date, end_date, save_dir=None):
        """Save OHLCV data for a single KRW ticker from Upbit exchange from start date to end date.

        Args:
            ticker (str): The ticker symbol to save data.
            start_date (datetime): The start date from which to begin saving data.
            end_date (datetime): The end date till which the data needs to be saved.
            save_dir (str): The directory in which to save the data. If None, use the save_path from the constructor.
        """
        if save_dir is None:
            save_dir = self.save_path
        ...
        print('===========================================')
        print(ticker)
        adjusted_start_date = self.get_start_date(ticker, start_date)
        if adjusted_start_date > end_date:
            print(f"No data available for {ticker} between the specified dates.")
            return
        print(adjusted_start_date)
        time.sleep(1)
        adjusted_end_date = min(end_date, datetime.now())
        self.save_ticker_data(ticker, adjusted_start_date, adjusted_end_date, save_dir)
        time.sleep(1)