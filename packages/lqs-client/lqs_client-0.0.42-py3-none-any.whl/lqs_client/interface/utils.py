...
class Utils:
    ...
    def get_image_from_ark_json_msg(self, message, image_mode=None):
        data = bytes(message["data"])

        if image_mode is None:
            image_mode = message["data_format"]

        image_mode = image_mode.lower()

        if image_mode == "depthz16":
        ...
        if image_mode == "bgr":
            b, g, r = img.split()
            img = ImagePIL.merge("RGB", (r, g, b))
        elif image_mode == "bgra":
            b, g, r, a = img.split()
            img = ImagePIL.merge("RGBA", (r, g, b, a))
        return img

    def get_image_from_ark_record(self, record, image_mode=None):
        ...
        img = self.get_image_from_ark_json_msg(ark_json_msg, image_mode=image_mode)
        return img
