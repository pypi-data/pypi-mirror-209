    ...
    def run(self):
        try:
            if self.platform == "android":
                ...
                logFile = open(os.path.join(self.output_path, "py_ios_device.log"), 'w+', encoding='utf-8')
                self.monitor_pro  = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=logFile, stderr=logFile, shell=True)
                self.monitor_pro.wait()
    ...
    def stop(self):
        try:
            ...
            if self.platform == 'android' or self.platform == "windows":
                ...
                try:
                    self.monitor_pro.stdin.write(b'exit\n')
                    self.monitor_pro.communicate(timeout=60)
