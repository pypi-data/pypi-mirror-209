...
if [[ ! -d $(rustc --print target-libdir --target powerpc64le-unknown-linux-gnu) ]]; then rustup target add powerpc64le-unknown-linux-gnu; fi
echo "::endgroup::"
maturin build --target powerpc64le-unknown-linux-gnu -i python3.9 --release --out dist