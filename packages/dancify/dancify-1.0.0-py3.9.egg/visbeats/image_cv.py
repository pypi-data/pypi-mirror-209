...
import visbeats.image as vbImage
import math
...
if(USING_OPENCV):
    ...
    def cvGoodFeaturesToTrack(self, maxCorners=None, qualityLevel=None, minDistance=None, corners=None, mask=None, blockSize=None, useHarrisDetector=None):
        ...
        if(maxCorners is not None):
            argd['maxCorners']=maxCorners
        if(qualityLevel is not None):
            argd['qualityLevel']=qualityLevel
        if(minDistance is not None):
            argd['minDistance']=minDistance
        if(corners is not None):
            argd['corners']=corners
        if(mask is not None):
            argd['mask']=mask
        if(blockSize is not None):
            argd['blockSize']=blockSize
        if(useHarrisDetector is not None):
            argd['useHarrisDetector']=useHarrisDetector
