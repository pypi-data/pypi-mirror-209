...
class Image(AObject):
    ...
    def _pixels_float(self):
        if (self._is_float):
            ...
            return self.data.astype(float) * np.true_divide(1.0, 255.0)
    ...
    def getScaled(self, shape=None, shape_xy=None):
        ...
        # imK = resize(self.data, sz)
        imK = np.array(PIM.fromarray(self.data).resize((sz[0], sz[1]), PIM.BICUBIC))
...
import visbeats.image_cv as Image_CV
