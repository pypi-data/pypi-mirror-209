    ...
    def list(params, user_id):
        ...
        data = filter_result_field(
            result_list=filter_result_field(  # 扩展字段替换
                result_list=data,
                alias_dict={"full_name": "their_full_name", "user_name": "their_user_name",
        ...
        # extend_id_list = [item.get("id", None) for item in data]
        # extend_list, err = FianceExtendService.get_extend_info(extend_id_list)
        # if extend_list:
        #     data = JoinList(data, extend_list, "id", "finance_id").join()
