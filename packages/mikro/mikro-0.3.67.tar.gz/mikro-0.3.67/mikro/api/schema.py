...
from typing import Dict, Optional, List, Iterator, AsyncIterator, Literal, Tuple
...
class Search_graphQueryOptions(BaseModel):
...
class Search_graphQuery(BaseModel):
    options: Optional[Tuple[Optional[Search_graphQueryOptions], ...]]
    "My samples return all of the users samples attached to the current user"
    ...
    class Meta:
        document = "query search_graph($search: String, $values: [ID]) {\n  options: graphs(name: $search, ids: $values) {\n    value: id\n    label: name\n  }\n}"
...
async def asearch_graph(
    ...
    values: Optional[List[Optional[ID]]] = None,
    rath: MikroRath = None,
) -> Optional[List[Optional[Search_graphQueryOptions]]]:
    """search_graph
    ...
    Returns:
        Optional[List[Optional[Search_graphQueryGraphs]]]"""
    return (
        await aexecute(
            Search_graphQuery, {"search": search, "values": values}, rath=rath
        )
    ).graphs
