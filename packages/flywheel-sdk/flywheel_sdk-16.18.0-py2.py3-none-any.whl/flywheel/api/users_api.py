...
class UsersApi(object):
    ...
    def get_current_user_key(self, **kwargs):  # noqa: E501
        """Generates user api key

        Generates user api key  Args:     auth_session (AuthSession): auth session of user Returns:     ApiKey (ApiKey): Pydantic model representing api key
        This method makes a synchronous HTTP request by default.
        ...
        if kwargs.get('async_'):
            return self.get_current_user_key_with_http_info(**kwargs)  # noqa: E501
        else:
            (data) = self.get_current_user_key_with_http_info(**kwargs)  # noqa: E501
            if (
                data
                and hasattr(data, 'return_value')
                and not ignore_simplified_return_value
    ...
    def get_current_user_key_with_http_info(self, **kwargs):  # noqa: E501
        """Generates user api key
        ...
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_current_user_key" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        return self.api_client.call_api(
            ...
            post_params=form_params,
            files=local_var_files,
            response_type='LegacyApiKey',  # noqa: E501
            auth_settings=auth_settings,
            async_=params.get('async_'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
