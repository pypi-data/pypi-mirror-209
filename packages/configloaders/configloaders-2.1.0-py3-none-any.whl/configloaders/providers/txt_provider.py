...
class TXTProvider(FileProvider):
    ...
    def read(self, path: pathlib.Path) -> dict:
        with open(path, 'r') as file: return {self.key(i): line for i, line in enumerate(file.readlines())}

    def write(self, path: pathlib.Path, data: dict) -> None:
        with open(path, 'w') as file: file.writelines([str(line) for i, line in data.items() if isinstance(i, int)])
