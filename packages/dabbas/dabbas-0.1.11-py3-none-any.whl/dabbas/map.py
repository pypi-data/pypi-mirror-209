...
def read_csv(file, latitude='latitude', longitude='longitude', geometry=['geometry', 'geom', 'wkb_geometry'], format='', **kwargs) -> gpd.GeoDataFrame:
...
def read_sql(sql, database=database_url(), latitude='latitude', longitude='longitude', geometry=['geometry', 'geom', 'wkb_geometry'], format='', **kwargs) -> gpd.GeoDataFrame:
...
def map_it(df: pd.DataFrame, format='', tooltip='name', geometry=['geometry', 'geom', 'wkb_geometry'], map=None, color='green', colormap='viridis', heatmap=None, opacity=0.5, zoom_level=5, center=[20.5937, 78.9629]):
    ...
    def style_function(x):
        return {
            "fillColor": get_color(float(x['id'])),
            "fillOpacity": opacity,
        }
        ...
        if not row[geometry].is_empty:
            ...
            if isinstance(geom, Point):
                ...
                marker(location=[row[geometry].y, row[geometry].x],
                       tooltip=_tooltip, color=point_color).add_to(_map)
            if isinstance(geom, Polygon) or isinstance(geom, MultiPolygon):
                ...
                folium.GeoJson(data=gpd.GeoSeries(
                    geom, [0] if heatmap is None else [row[heatmap]]).to_json(), tooltip=_tooltip, style_function=style_function).add_to(_map)
        else:
            errs.append(idx)
