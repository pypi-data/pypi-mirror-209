...
def test_data_lake_producer_central_queue(
    ...
    producer.publish_data_lake_csv_file(csv_file_path)
    queue_consumer.expect_messages(1)
    producer.publish_data_lake_json_file(json_file_path)
    queue_consumer.expect_messages(2)
    with open(json_file_path, "rb") as f:
        producer.publish_data_lake_json(json.load(f))
    producer.publish_data_lake_parquet_file(parquet_file_path)
