...
class DataLakeDataProducer:
    ...
    @instrumented
    def publish_data_lake_csv_file(self, file_path: str):
        with open(file_path, "rb") as f:
            csv_data = f.read()
            self.publish_data_lake_csv(csv_data)

    @instrumented
    def publish_data_lake_csv(self, csv_data: bytes):
        serialized_data, compression = self._serialize_data(csv_data)
        self.publish_data_lake_data("CSV", serialized_data, compression)

    @instrumented
    def publish_data_lake_json_file(self, file_path: str):
        with open(file_path, "rb") as f:
            json_data = json.load(f)
            self.publish_data_lake_data("JSON", json_data)

    @instrumented
    def publish_data_lake_json(self, json_data: dict):
        self.publish_data_lake_data("JSON", json_data)

    @instrumented
    def publish_data_lake_parquet_file(self, file_path: str):
        with open(file_path, "rb") as f:
            serialized_data, compression = self._serialize_data(f.read())
            self.publish_data_lake_data("PARQUET", serialized_data, compression)
