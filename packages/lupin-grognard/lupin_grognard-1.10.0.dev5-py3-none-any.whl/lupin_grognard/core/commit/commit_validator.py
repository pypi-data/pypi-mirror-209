...
class CommitValidator(Commit):
    ...
    def _validate_body(self) -> bool:
        ...
        if self._is_ci_mr_target_branch_main():
    ...
    def _is_ci_mr_target_branch_main(self) -> bool:
        print(f"CI_MERGE_REQUEST_TARGET_BRANCH_NAME: {os.environ.get('CI_MERGE_REQUEST_TARGET_BRANCH_NAME')}")
        print(f"CI_MERGE_REQUEST_SOURCE_BRANCH_NAME: {os.environ.get('CI_MERGE_REQUEST_SOURCE_BRANCH_NAME')}")
        return os.environ.get('CI_MERGE_REQUEST_TARGET_BRANCH_NAME') == "main"

    def _is_last_description_line_jama_reference(self) -> bool:
        regex = r"JAMA-SmlPrep-SUBSR-\d+"
        return bool(re.match(regex, self.body[-1]))
