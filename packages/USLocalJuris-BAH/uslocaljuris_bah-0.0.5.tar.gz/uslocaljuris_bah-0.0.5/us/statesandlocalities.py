...
class State(object):
    ...
    def shapefile_urls(self, region=None):
        ...
        #https://www2.census.gov/geo/tiger/TIGER2020/TRACT/tl_2020_01_tract.zip
            "tract": "{0}/TRACT/tl_2020_{1}_tract20.zip".format(
                base_url, self.fips
            ),
            "cd": "{0}/CD/CD118/tl_2020_{1}_cd118.zip".format(base_url, self.fips),
            "county": "{0}/COUNTY/tl_2020_{1}_county20.zip".format(
                base_url, self.fips
            ),
            "state": "{0}/STATE/tl_2020_{1}_state20.zip".format(
                base_url, self.fips
            ),
            #URL for ZCTA has changed
            "zcta": "{0}/ZCTA5/tl_2020_{1}_zcta520.zip".format(
                base_url, self.fips
            ),
            "block": "{0}/TABBLOCK/tl_2020_{1}_tabblock20.zip".format(
                base_url, self.fips
            ),
            "blockgroup": "{0}/BG/tl_2020_{1}_bg20.zip".format(
                base_url, self.fips
            ),
        }
...
def lookup(val, field=None, use_cache=True):
    ...
    for state in itertools.chain(STATES_AND_TERRITORIES, [DC], OBSOLETE, LOCALITIES):
...
def mapping(from_field, to_field, states=None):
    if states is None:
        states = itertools.chain(STATES_AND_TERRITORIES, [DC], OBSOLETE,LOCALITIES)
...
KC = State(
    **{
        ...
        "is_locality": True,
        "is_city":True,
        "part_of_state": "Missouri",
        "statehood_year": None,
        "capital": "Kansas City",
        "capital_tz": "America/Chicago",
        "ap_abbr": "KSC",
        "time_zones": ["America/Chicago"],
        "name_metaphone": "KNSSST",
    }
)
