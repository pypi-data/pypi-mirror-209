...
class SilaServer:
    ...
    def __start(
        ...
        *,
        ...
        self.__collect_calls_affected_by_metadata()  # not yet used
    ...
    def __collect_calls_affected_by_metadata(self) -> Dict[Union[Command, Property], List[Metadata]]:
        calls_affected_by_metadata = defaultdict(list)

        for meta in self.__get_all_implemented_metadata():
            for call in self.__get_calls_affected_by_metadata(meta):
                if isinstance(call, FullyQualifiedIdentifier):
                    call = self.children_by_fully_qualified_identifier[call]

                if isinstance(call, (Command, Property)):
                    calls_affected_by_metadata[call].append(meta)
                elif isinstance(call, Feature):
                    for target in _get_commands_and_properties(call):
                        calls_affected_by_metadata[target].append(meta)
                else:
                    raise ValueError(
                        f"Only command, properties and feature can be affected by metadata, not {type(call)}"
                    )

        return calls_affected_by_metadata

    def __get_all_implemented_metadata(self) -> List[Metadata]:
        metadata = []
        for feature in self.features.values():
            for meta in feature.metadata_definitions.values():
                metadata.append(meta)
        return metadata

    def __get_calls_affected_by_metadata(
        self, meta: Metadata
    ) -> List[Union[Feature, Command, Property, FullyQualifiedIdentifier]]:
        affected_calls_method = getattr(
            self.feature_servicers[meta.parent_feature._identifier].implementation,
            "get_calls_affected_by_" + meta._identifier,
        )
        return affected_calls_method()


def _get_commands_and_properties(feature: Feature) -> List[Union[Command, Property]]:
    children = []
    for unob_cmd in feature._unobservable_commands.values():
        children.append(unob_cmd)
    for ob_cmd in feature._observable_commands.values():
        children.append(ob_cmd)
    for unob_prop in feature._unobservable_properties.values():
        children.append(unob_prop)
    for ob_prop in feature._observable_properties.values():
        children.append(ob_prop)
    return children
