...
def is_http_url(url: str, logger: Logger = None) -> bool:
    """
    ...
    """
    if url is None:
        print_info(logger, "URL is None")
        return False
    if len(url) == 0:
        print_info(logger, "URL is empty")
        return False

    print_info(logger, "Testing if URL '{}' is HTTP(s) URL".format(url))

    if not re.search(__URL_REGEX, url):
        print_warn(logger, "URL '{}' does not match URL's regexp".format(url))
        return False
    ...
    try:
        ...
        print_err(logger, "Cannot parse URL {}: {}".format(url, ex))
        return False

    if not uri.scheme:
        print_warn(logger, "Scheme is undefined for URL {}.".format(url))
        return False
    if not uri.scheme.lower() in ['http', 'https']:
        print_warn(logger, "Scheme is not HTTP(s) for URL {}.".format(url))
        return False
    if not uri.hostname:
        print_warn(logger, "Host is undefined for URL {}.".format(url))
        return False
...
def get_url_retry_on_client_errors(url: str,
                                   ...
                                   logger: Logger = None) -> AbstractWebClientResponse:
    """
    ...
    for retry in range(0, retry_count):
        print_info(logger, "Fetching URL {}...".format(url))
        response = web_client.get(url)

        if isinstance(response, WebClientErrorResponse):
            print_warn(logger, "Request for URL {} failed: {}".format(url, response.message()))

            if response.retryable():
                print_info(logger, "Retrying URL {} in {} seconds...".format(url, sleep_between_retries))
                time.sleep(sleep_between_retries)

            else:
                print_info(logger, "Not retrying for URL {}".format(url))
                return response

        else:
            return response

    print_info(logger, "Giving up on URL {}".format(url))
    return response
...
def ungzipped_response_content(url: str,
    ...
    if __response_is_gzipped_data(url=url, response=response):
        try:
            ...
            print_err(logger, "Unable to gunzip response {}, maybe it's a non-gzipped sitemap: {}"
                       .format(response, ex))
