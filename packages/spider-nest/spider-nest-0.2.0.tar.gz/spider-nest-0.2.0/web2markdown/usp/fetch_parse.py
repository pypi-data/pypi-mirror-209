...
class SitemapFetcher(object):
    ...
    def __init__(self, url: str, recursion_level: int,
        ...
        if not is_http_url(url, logger=logger):
    ...
    def sitemap(self) -> AbstractSitemap:
        print_info(self._logger, "Fetching level {} sitemap from {}...".format(self._recursion_level, self._url))
        response = get_url_retry_on_client_errors(url=self._url, web_client=self._web_client, logger=self._logger)
        ...
        response_content = ungzipped_response_content(url=self._url, response=response, logger=self._logger)
        ...
        print_info(self._logger, "Parsing sitemap from URL {}...".format(self._url))
    ...
    def sitemap(self) -> AbstractSitemap:
        ...
        for robots_txt_line in self._content.splitlines():
            ...
            if sitemap_match:
                ...
                if is_http_url(sitemap_url, logger=self._logger):
                    sitemap_urls[sitemap_url] = True
                else:
                    print_warn(self._logger, "Sitemap URL {} doesn't look like an URL, skipping".format(sitemap_url))
...
class PlainTextSitemapParser(AbstractSitemapParser):
    ...
    def sitemap(self) -> AbstractSitemap:
        ...
        for story_url in self._content.splitlines():
            ...
            if not story_url:
                continue
            if is_http_url(story_url, logger=self._logger):
                story_urls[story_url] = True
            else:
                print_warn(self._logger, "Story URL {} doesn't look like an URL, skipping".format(story_url))
...
class XMLSitemapParser(AbstractSitemapParser):
    ...
    def sitemap(self) -> AbstractSitemap:
        ...
        try:
            ...
            print_err(self._logger, "Parsing sitemap from URL {} failed: {}".format(self._url, ex))
...
class IndexXMLSitemapParser(AbstractXMLSitemapParser):
    ...
    def xml_element_end(self, name: str) -> None:
        ...
        if name == 'sitemap:loc':
            ...
            if not is_http_url(sub_sitemap_url, logger=self._logger):
                print_warn(self._logger, "Sub-sitemap URL does not look like one: {}".format(sub_sitemap_url))
...
class PagesXMLSitemapParser(AbstractXMLSitemapParser):
    ...
    class Page(object):
        ...
        def page(self) -> Optional[SitemapPage]:
            ...
            if not url:
                print_err(self.logger, "URL is unset")
                return None
            ...
            if change_frequency:
                ...
                if SitemapPageChangeFrequency.has_value(change_frequency):
                    ...
                    print_warn(self.logger, "Invalid change frequency, defaulting to 'always'."
                               .format(change_frequency))
            ...
            if priority:
                ...
                if comp_zero in (Decimal('0'), Decimal('1') and comp_one in (Decimal('0'), Decimal('-1'))):
                    ...
                    print_warn(self.logger, "Priority is not within 0 and 1: {}".format(priority))
    ...
    def xml_element_start(self, name: str, attrs: Dict[str, str]) -> None:
        ...
        if name == 'sitemap:url':
            ...
            self._current_page = self.Page(logger=self._logger)
...
class PagesRSSSitemapParser(AbstractXMLSitemapParser):
    ...
    class Page(object):
        ...
        def page(self) -> Optional[SitemapPage]:
            ...
            if not link:
                print_err(self.logger, "Link is unset")
                return None

            title = html_unescape_strip(self.title)
            description = html_unescape_strip(self.description)
            if not (title or description):
                print_err(self.logger, "Both title and description are unset")
                return None
