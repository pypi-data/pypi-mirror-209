...
from VSautomatic.TestCfgParse import TestCfgParse,CfgParse
...
# dutIP = restCfg.GetCfgInfo("RestConnection","TV.IP") #DUT IP Address
# print("DUT IP address:{}".format(dutIP))
# tableIP = restCfg.GetCfgInfo("Tablet","Tablet_IP") #DUT IP Address
# print("Table IP address:{}".format(tableIP))
# PORT=restCfg.GetCfgInfo("RestConnection","Port") # REST Port
# print("REST Port:{}".format(PORT))
...
class restterminal():
    ...
    # def pair(self):
        ...
        # subprocess.call(["adb.exe", "disconnect"], shell=True)
        # subprocess.call(["adb.exe", "connect", self.ip], shell=True)
        # # subprocess.call(["adb", "connect", tableIP], shell=True)
        # process = subprocess.Popen(["adb.exe", "-s", self.ip+":5555", "shell", "logcat"], stdout=subprocess.PIPE, shell=True)
        # stdout_queue = queue.Queue()
        # stdout_reader = AsynchronousFileReader(process.stdout, stdout_queue)
        # stdout_reader.start()
        # # find challenge_type and req_token
        # uri = "https://" + self.ip + ":" + PORT + "/pairing/start"
        ...
        #             uri = "https://" + self.ip + ":" + PORT + "/pairing/pair"
    ...
    def sshpair(self):
        ...
        # force requires_pairing and enforce_auth_token to be true
        # start adb logcat thread
        self.process = subprocess.Popen(["ssh", "-o", "StrictHostKeyChecking no", "root@" + self.ip, "logcat", "-t 2000"], stdin=subprocess.PIPE,
        ...
        if result["STATUS"]["RESULT"] != "SUCCESS":
            print(("Can't connect to TV. STATUS: %s" % result["STATUS"]["RESULT"]))
            # wx.CallAfter(self.window.runCase.Enable, True)

        challenge_type = result["ITEM"]["CHALLENGE_TYPE"]
        req_token = result["ITEM"]["PAIRING_REQ_TOKEN"]
        # find challenge pin and check if it gives a valid authentication token
        outputstring = self.process.communicate()[0].decode()
        # print("{}".format(outputstring))
        outputlist = outputstring.splitlines()
        # print("{}".format(outputlist))
        for line in outputlist:
            if "PingCode" in line:
                trash, lines = line.split("PingCode", 1)
                pins = lines.split()
                challenge_pin = pins[0]
                print("{}".format(challenge_pin))
                uri = "https://" + self.ip + ":" + PORT + "/pairing/pair"
                payload = json.dumps({"DEVICE_ID": DEVICE_ID, "CHALLENGE_TYPE": challenge_type,
                                      "RESPONSE_VALUE": challenge_pin, "PAIRING_REQ_TOKEN": req_token})
                result = requests.put(uri, verify=False, data=payload, headers=HEADERS).json()
                print("{}".format(result))
                if (result["STATUS"]["RESULT"] == "SUCCESS"):
                    code = result["ITEM"]["AUTH_TOKEN"]
                    return code
                
    def sshpair_close(self):
        # self.stdout_reader.join()
        self.process.stdout.close()

    def read_dictionary(self):
        global dict
        file = open(log_file_path + '\\Dictionary.txt', 'r')
        for line in file:
            ...
            dict[key] = "https://" + self.ip + ":" + PORT + partial_uri
    ...
    def parse_command(self,line,auth_token):
        ...
        # static get
            ...
            if len(values) > 0:
                restlog_file = values[0]
            else:
                restlog_file = log_file_path + "/restlog.txt"

            uri = "https://" + self.ip + ":" + PORT + "/restlog"
...
if __name__ == "__main__":
    rest = restterminal()
    auth_token = rest.sshpair()
    # print(auth_token)
    rest.read_dictionary()
    # requests.packages.urllib3.disable_warnings()
    rest.parse_command("launch youtube", auth_token)
