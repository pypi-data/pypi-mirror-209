...
class CIS:
    ...
    def viewOwnerHistoryRequest(self, tokenID: str):
        sp = io.BytesIO()
        # sp.write(leb128.u.encode(contract_index))
        # sp.write(leb128.u.encode(contract_subindex))
        sp.write(self.generate_tokenID(tokenID))
        return sp.getvalue()
        # return base58.b58encode_check(b"\x02" + sp.getvalue())
    ...
    def viewOwnerHistoryResponse(self, res: bytes):
        ...
        results = []
        for _ in range(0, n):
            results.append(self.account_address(bs))
