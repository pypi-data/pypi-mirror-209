...
class HierarchicalView():
    ...
    def _superresolve_embeddings(self, n_epochs=10000, tol=1e-4, update_alg='gd'):
        ...
        for dataset_index, dataset in enumerate(self.datasets):
            ...
            X_B = self.low_res_view.embedding_optimizer.embedding_state[dataset.name].to(self.context["device"])
...
def construct_hierarchy(base_view: HierarchicalView, chunks: int = 16, 
    ...
    for level in range(1, levels):
        ...
        binned_datasets = []
        binned_Ys = []
        previous_Ys = base_view.Ys
        for previous_Y, dataset in zip(previous_Ys, previous_datasets):
            binned_dataset = _spatial_binning(dataset, chunks=chunks, chunk_size=chunk_size, chunk_1d_density=chunk_1d_density)
