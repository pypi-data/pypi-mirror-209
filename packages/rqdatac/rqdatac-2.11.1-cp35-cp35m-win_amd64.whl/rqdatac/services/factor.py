...
def get_factor_return(
    start_date, end_date, factors=None, universe="whole_market", method="implicit", industry_mapping="sw2021", model="v1", market="cn"
):
    """获取因子收益率数据
    ...
    :param industry_mapping(str): 使用的行业类别，可选 sw2021, citics2019, 默认为 sw2021
...
def get_factor_exposure(order_book_ids, start_date=None, end_date=None, factors=None, industry_mapping='sw2021', model="v1", market="cn"):
    ...
    check_items_in_container(industry_name, ["sw2014", "sw2021", "citics2019"], "industry_mapping")
    check_items_in_container(model, ["v1", "v2"], "model")
...
def get_style_factor_exposure(order_book_ids, start_date, end_date, factors=None, model="v1",
                              industry_mapping="sw2021", market="cn"):
    """获取个股风格因子暴露度
...
def get_eigenfactor_adjusted_covariance(date, horizon='daily', model="v1", industry_mapping="sw2021"):
    """ 获取因子协方差矩阵（特征因子调整）

    :param date: str 日期（例如：‘2017-03-20’）
    :param horizon: str 预测期限。默认为日度（'daily'），可选月度（‘monthly’）或季度（'quarterly'）。
    :param model: 选用的模型, 可选模型包括 v1, v2; 默认为 v1
    :param industry_mapping str                     所选用的行业映射, 可选值包括 sw2021
...
@export_as_api
def get_factor_covariance(date, horizon='daily', model="v1", industry_mapping="sw2021"):
    """ 获取因子协方差矩阵
...
@export_as_api
def get_specific_return(order_book_ids, start_date, end_date, model="v1", industry_mapping="sw2021"):
    """ 获取个股特异收益率
...
@export_as_api
def get_specific_risk(order_book_ids, start_date, end_date, horizon='daily', model="v1", industry_mapping="sw2021"):
    """ 获取个股特异波动率(标准差)
...
def get_cross_sectional_bias(start_date, end_date, type='factor', model="v1", industry_mapping="sw2021"):
    """ 获取横截面偏差系数
