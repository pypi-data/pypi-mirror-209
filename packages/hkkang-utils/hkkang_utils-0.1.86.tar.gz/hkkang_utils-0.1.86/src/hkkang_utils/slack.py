...
def slack_notification(channel: str, success_msg: str, error_msg: str, token: str = DEFAULT_ACCESS_TOKEN):
    slack_messenger = SlackMessenger(channel=channel, token=token)
    try:
        yield slack_messenger
        slack_messenger.send(success_msg)
    except Exception as e:
        slack_messenger.send(f"{error_msg} with {e.__class__.__name__} error: {e}")
        raise e