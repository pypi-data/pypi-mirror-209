...
cpdef inline deserialize(x):
    cdef core.K kx = core.kpn(<char*>x, len(x))
...
def _pyfactory(addr: int, incref: bool, typenum: int):
    ...
    if 'np' in k_dir: # nocov
        return k_object.np() # nocov
    elif 'py' in k_dir: # nocov
        return k_object.py() # nocov
    elif 'pd' in k_dir: # nocov
        return k_object.pd() # nocov
    elif 'pa' in k_dir: # nocov
        return k_object.pa() # nocov
