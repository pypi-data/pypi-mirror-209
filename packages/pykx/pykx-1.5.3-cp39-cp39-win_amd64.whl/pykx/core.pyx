...
def _normalize_qargs(user_args: Tuple[str]) -> Tuple[bytes]:
...
if qinit_check_data is not None:                                                   # nocov
    ...
    import shlex                                                                   # nocov
    _qargs = tuple(shlex.split(_qargs))                                            # nocov
...
if under_q: # nocov
    ...
    if '--unlicensed' in qargs:
        ...
        if _qinit_check_proc.returncode: # Fallback to unlicensed mode
            ...
            qinit_return_code = _qinit(qinit, str(qhome if ignore_qhome else pykx_lib_dir), str(qlic), qargs)
