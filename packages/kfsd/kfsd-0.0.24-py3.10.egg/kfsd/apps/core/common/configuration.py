...
class Configuration:
    def __init__(self, **kwargs):
        self.__rawSettings = {
            tuple(item["setting"]): item for item in kwargs.pop("settings")
        }
        self.__reqKeyList = [
...
class KubefacetsConfig:
    ...
    def deriveConfig(self):
        ...
        isLocalConfig = DictUtils.get_by_path(
            kubefacetsSettings, "config.is_local_config"
        )
        lookupDimensions = DictUtils.get_by_path(
            kubefacetsSettings, "config.lookup_dimension_keys"
        )
        if isLocalConfig:
