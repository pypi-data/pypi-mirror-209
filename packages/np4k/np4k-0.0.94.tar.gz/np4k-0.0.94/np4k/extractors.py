...
class ContentExtractor(object):
    ...
    def get_publishing_date(self, url, doc):
        ...
        if date_match:
            ...
            if datetime_obj:
                return_datetime_obj = datetime_obj
                #return datetime_obj
        ...
        #TODO: ideally we would check to see if approach 1 or 2 above gave a more specific datetime obj, and return that one.
        return return_datetime_obj
    ...
    def get_title(self, doc):
        ...
        # if title is too short, use title_text_h1. e.g. Financial Times
        if title != "" and len(title.split(" ")) < 3 and title_text_h1 != "":
            title = title_text_h1

        # if title is empty, use title_text_h1
        if title == "" and title_text_h1 is not None:
            title = title_text_h1
    ...
    def get_category_urls(self, source_url, doc):
        ...
        for p_url in page_urls:
            ...
            if scheme and (scheme != 'http' and scheme != 'https'):
                if self.config.verbose:
                    print(f"elim category url {p_url} for bad scheme, not http nor https")
                continue

            if domain:
                child_tld = tldextract.extract(url=p_url)
                domain_tld = tldextract.extract(url=source_url)
                ...
                for part in child_subdomain_parts:
                    if part == domain_tld.domain:
                        if self.config.verbose:
                            print(f"subdomain contains at {part} and {domain_tld.domain}")
                ...
                if not subdomain_contains and (child_tld.domain != domain_tld.domain):
                    ...
                    if self.config.verbose:
                        print(f"elim category url {p_url} for domain mismatch")
                    continue
                valid_categories.append(f"{scheme}://{domain}")
                ...
                path_chunks = [chunk for chunk in path.split('/') if len(chunk) > 0 and chunk != 'index.html']

                # bug: this misses a lot of cases
                if len(path_chunks) == 1 and len(path_chunks[0]) < 14:
                    valid_categories.append(f"{domain}{path}")
                else:
                    if self.config.verbose:
                        print(f"elim category url {p_url} for >1 path chunks or size path chunks")
