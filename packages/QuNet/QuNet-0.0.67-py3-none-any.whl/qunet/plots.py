...
def plot_smooth_line(ax=None, x=[],y=[], color="black", label="", count=200, kern=51, stride=1, width=1.5, alpha=0.5):    
    if len(x) != len(y):        
    ...
    if len(x) <  count:
        ...
        avg_x = np.append(avg_x, [ x[-1] ])            # экстраполируем вперёд
        x1,x2,x3 = avg_x[-3], avg_x[-2], avg_x[-1]     # даже при stride=1 график отстаёт 
        y1,y2    = avg_y[-2], avg_y[-1]                # из-за усреднения x
        avg_y = np.append(avg_y, [((x3-x1)*y2 - (x3-x2)*y1)/(x2-x1) if x2 != x1 else y2])
