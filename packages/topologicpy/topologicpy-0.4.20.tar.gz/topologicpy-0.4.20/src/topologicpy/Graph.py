...
class Graph:
    @staticmethod
    def AdjacencyMatrixOld(graph, edgeKey=None, bidirectional=True, tolerance=0.0001):
        """
        ...
        edgeKey : str , optional
            If set, the value at this key in the connecting edge will be used instead of the value 1. The default is None.
        bidirectional : bool , optional
            If set to True, the edges in the graph will be treated as being bidirectional. Otherwise, the start vertex and end vertex of the connecting edge will determine the direction. The default is True.
    ...
    @staticmethod
    def CriticalPath(graph, edgeKey=None, tolerance=0.0001):
        """
        Returns the critical path of the input graph. See https://en.wikipedia.org/wiki/Critical_path_method
        ...
        edgeKey : string , optional
            The edge dictionary key to use. This value should be numeric and should represent weights or durations (e.g. the time it takes to complete a task). If set to None, the length of the edge will be used instead.
        tolerance : float , optional
            The desired tolerance. The default is 0.0001
        ...
        -------
        topologic.Wire
            The wire representing the critical path.
        
        """
        from topologicpy.Wire import Wire
        def compute_critical_path(adjacency_matrix):
            n = len(adjacency_matrix)
            
            # Step 1: Calculate the earliest start time for each node
            earliest_start_time = [0] * n
            for i in range(n):
                for j in range(n):
                    if adjacency_matrix[i][j] != 0:
                        earliest_start_time[j] = max(earliest_start_time[j], earliest_start_time[i] + adjacency_matrix[i][j])
            
            # Step 2: Calculate the latest start time for each node
            latest_start_time = [float('inf')] * n
            latest_start_time[n-1] = earliest_start_time[n-1]  # Set the latest start time of the last node
            
            for i in range(n-2, -1, -1):
                for j in range(n):
                    if adjacency_matrix[i][j] != 0:
                        latest_start_time[i] = min(latest_start_time[i], latest_start_time[j] - adjacency_matrix[i][j])
            ...
            for i in range(n):
                for j in range(n):
                    if adjacency_matrix[i][j] != 0:
                        if earliest_start_time[i] + adjacency_matrix[i][j] == latest_start_time[j]:
        ...
        if edgeKey == None:
            ...
            useEdgeLength = False
        adjMatrix = Graph.AdjacencyMatrix(graph, edgeKeyFwd=edgeKey, edgeKeyBwd=None, bidirKey=None, bidirectional=False, useEdgeIndex = False, useEdgeLength = useEdgeLength, tolerance=tolerance)
        edgeMatrix = Graph.AdjacencyMatrix(graph, edgeKeyFwd=None, edgeKeyBwd=None, bidirKey=None, bidirectional=False, useEdgeIndex = True, useEdgeLength = False, tolerance=tolerance)
        print(edgeMatrix)
        cp = compute_critical_path(adjMatrix)
        print("CP", cp)
        cp_edges = []
        selectors = []
        for item in cp:
            sv = item[0]
            ev = item[1]
            edge = edges[edgeMatrix[sv][ev]-1]
            selector = Topology.Centroid(edge)
            selector = Topology.SetDictionary(selector, Topology.Dictionary(edge))
            selectors.append(selector)
            cp_edges.append(edge)
        print(cp_edges)
        wire = Wire.ByEdges(cp_edges)
        wire = Topology.TransferDictionariesBySelectors(wire, selectors, tranEdges=True)
        return wire
    ...
    @staticmethod
    def LongestPath(graph, vertexA, vertexB, vertexKey=None, edgeKey=None, costKey=None, tolerance = 0.0001):
        """
        ...
        costKey : str , optional
            The dictionary key under which to store the total cost of the returned path. If set to None, no cost will not be stored in the returned path. The default is None.
        tolerance : float , optional
            The desired tolerance. The default is 0.0001
        ...
        def longerThan(adjMatrix, pathA, pathB, vertices):
            vertSumA = 0
            edgeSumA = 0
            for i in range(len(pathA)-1):
                vertSumA += vertices[pathA[i]]
                edgeSumA += adjMatrix[pathA[i]][pathA[i+1]]
            if len(pathA) > 0:
                vertSumA += vertices[pathA[-1]]
            totalA = vertSumA + edgeSumA

            vertSumB = 0
            edgeSumB = 0
            for i in range(len(pathB)-1):
                vertSumB += vertices[pathB[i]]
                edgeSumB += adjMatrix[pathB[i]][pathB[i+1]]
            if len(pathB) > 0:
                vertSumB += vertices[pathB[-1]]
            totalB = vertSumB + edgeSumB
            return [totalA > totalB, max([totalA, totalB])]
        
        def find_longest_path(adjMatrix, start, end, vertices):
            num_vertices = len(adjMatrix)
            visited = [False] * num_vertices  # List to keep track of visited vertices
            longest_path = []  # Variable to store the longest path
            ...
            def dfs(current_vertex, current_path):
                ...
                current_path.append(current_vertex)  # Add the current vertex to the current path

                # Check if we reached the end vertex
                if current_vertex == end:
                    #if len(current_path) > len(longest_path):
                    is_longer, total_cost = longerThan(adjMatrix,current_path, longest_path, vertices)
                    if is_longer:
                        longest_path = current_path.copy()
                
                # Recursively visit all adjacent vertices that haven't been visited yet
                for neighbor in range(num_vertices):
                    if adjMatrix[current_vertex][neighbor] != 0 and not visited[neighbor]:
                        dfs(neighbor, current_path)
                
                # Backtrack: Remove the current vertex from the current path and mark it as unvisited
                current_path.pop()
                visited[current_vertex] = False
            
            dfs(start, [])
            
            return [longest_path, total_cost]
        ...
        if vertexKey == None:
            vertexValues = [0 for v in vertices]
        else:
            for v in vertices:
                d = Topology.Dictionary(v)
                value = Dictionary.ValueAtKey(d, vertexKey)
                if value == None:
                    vertexValues.append(0)
                else:
                    vertexValues.append(value)
        if edgeKey:
        ...
        for e in Graph.Edges(graph):
            es = Topology.Centroid(e)
            d = Topology.Dictionary(e)
            es = Topology.SetDictionary(es, d)
            edgeSelectors.append(es)
        start = Vertex.Index(vertexA, vertices)
        end = Vertex.Index(vertexB, vertices)
        adjMatrix = Graph.AdjacencyMatrix(graph, edgeKeyFwd=edgeKey, edgeKeyBwd=None, bidirKey=None, bidirectional=True, useEdgeIndex=False, useEdgeLength=useEdgeLength, tolerance=tolerance)
        edgeMatrix = Graph.AdjacencyMatrix(graph, edgeKeyFwd=edgeKey, edgeKeyBwd=None, bidirKey=None, bidirectional=True, useEdgeIndex = True, useEdgeLength=False, tolerance=tolerance)
        longest_path, total_cost = find_longest_path(adjMatrix, start, end, vertexValues)
        edgeList = []
        if len(longest_path) > 1:
            vertexList = [vertices[i] for i in longest_path]
            for i in range(len(longest_path)-1):
                edgeIndex = edgeMatrix[longest_path[i]][longest_path[i+1]]
                edgeList.append(edges[edgeIndex-1])
            wire = Topology.SelfMerge(Cluster.ByTopologies(edgeList))
            if not costKey == None:
                d = Dictionary.ByKeysValues([costKey], [total_cost])
    ...
    @staticmethod
    def ShortestPath(graph, vertexA, vertexB, vertexKey=None, edgeKey=None, costKey=None, tolerance = 0.0001):
        """
        ...
        vertexB : topologic.Vertex
            The second input vertex.
        vertexKey : str , optional
            The vertex key to minimize. If set the vertices dictionaries will be searched for this key and the associated value will be used to compute the shortest path that minimizes the total value. The value must be numeric. The default is None.
        edgeKey : str , optional
            The edge key to minimize. If set the edges dictionaries will be searched for this key and the associated value will be used to compute the shortest path that minimizes the total value. The value of the key must be numeric. If set to "length" (case insensitive), the shortest path by length is computed. The default is "length".
        costKey : str , optional
            The dictionary key under which to store the total cost of the returned path. If set to None, no cost will not be stored in the returned path. The default is None.
        tolerance : float , optional
            The desired tolerance. The default is 0.0001
        ...
        def find_shortest_path(graph, start, end, vertices):
            num_vertices = len(graph)
            distances = [math.inf] * num_vertices  # List to keep track of the shortest distances
            previous_vertices = [None] * num_vertices  # List to keep track of the previous vertices in the shortest path
            
            # Priority queue to store vertices with their respective distances
            priority_queue = [(0, start)]  # Tuple of (distance, vertex)
            distances[start] = 0
            
            while priority_queue:
                current_distance, current_vertex = heappop(priority_queue)
                
                # Skip if the current vertex has been visited with a shorter distance
                ...
                for neighbor in range(num_vertices):
                    edge_cost = graph[current_vertex][neighbor]
                    if edge_cost > 0:
                        distance = current_distance + edge_cost + vertices[neighbor]
                        
                        # Check if the new distance is shorter than the previously recorded distance
            ...
            return [shortest_path, distance+vertices[shortest_path[-1]]] #Account for the cost of the last vertex
        ...
        adjMatrix = Graph.AdjacencyMatrix(graph, edgeKeyFwd=edgeKey, edgeKeyBwd=None, bidirKey=None, bidirectional=True, useEdgeIndex=False, useEdgeLength=useEdgeLength, tolerance=tolerance)
        edgeMatrix = Graph.AdjacencyMatrix(graph, edgeKeyFwd=edgeKey, edgeKeyBwd=None, bidirKey=None, bidirectional=True, useEdgeIndex = True, useEdgeLength=False, tolerance=tolerance)
        shortest_path, total_cost = find_shortest_path(adjMatrix, start, end, vertexValues)
        edgeList = []
        if len(shortest_path) > 1:
            vertexList = [vertices[i] for i in shortest_path]
            for i in range(len(shortest_path)-1):
                edgeIndex = edgeMatrix[shortest_path[i]][shortest_path[i+1]]
                edgeList.append(edges[edgeIndex-1])
            wire = Topology.SelfMerge(Cluster.ByTopologies(edgeList))
            if not costKey == None:
                d = Dictionary.ByKeysValues([costKey], [total_cost])
