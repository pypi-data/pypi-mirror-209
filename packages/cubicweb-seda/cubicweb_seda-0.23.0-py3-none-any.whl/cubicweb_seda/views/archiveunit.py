...
class KeywordBusinessValueEntityView(viewlib.ListItemContentView):
    ...
    def entity_call(self, entity):
        ...
        self.w(
            '<span class="value">{} {}</span>'.format(msg, entity.view("seda.xsdmeta"))
        )
        ...
        if entity.reverse_seda_keyword_type_from:
            ...
            self.w("<br/><span>{} {}</span>".format(msg, kwt.view("seda.xsdmeta")))
...
class ReferenceListItemContentView(viewlib.ListItemContentView):
    ...
    def entity_call(self, entity):
        ...
        self.w("</b> :")
        alt_rtype = "seda_alt_{}_archive_unit_ref_id".format(
            un_camel_case(entity.cw_etype[4:])
        )
            ...
            if related:
                ...
                if rtype == "seda_event_type_from":
                    ...
                    if value:
                        value = value[0].label()
                attrs.append(
                    "{rtype} {value} {card}".format(
                        rtype=self._cw.__(rtype + "_object"), value=value, card=card
                    )
                )
        if attrs:
            self.w(" ({})".format(", ".join(attrs)))
        description = getattr(entity, "user_annotation", None)
        if description:
            self.w(' <div class="description text-muted">%s</div>' % description)
