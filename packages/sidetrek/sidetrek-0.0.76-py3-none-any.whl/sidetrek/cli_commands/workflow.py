...
def run(workflow_id: int = typer.Option(...), workflow_args: str = "{}"):
    ...
    * --workflow-args is a stringified JSON of your workflow arguments (e.g. '{"learning_rate"=0.1, "epochs"=5}').
    ...
    with Progress(
        ...
        print(f"[green]✔️ [white]Authenticated ({time_elapsed}s)")
        ...
        time_elapsed = time.time() - start_time
        print(f"[green]✔️ [white]Workflow generated ({time_elapsed}s)")
        ...
        with subprocess.Popen(
            ...
            if process.returncode != 0:
                ...
                print(f"[red]✕ [white]Workflow execution failed ({time_elapsed}s)")
                raise typer.Exit()

            progress.remove_task(wf_execution_step)
            time_elapsed = time.time() - start_time
            print(f"[green]✔️ [white]Workflow execution completed 🎉 ({time_elapsed}s)")
