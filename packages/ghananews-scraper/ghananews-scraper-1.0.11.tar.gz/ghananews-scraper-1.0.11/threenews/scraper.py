...
class ThreeNews:
    def __init__(self, author: str, limit_pages: int = None):
        ...
        if self.limit_pages is not None:
            ...
            print(f"Scraping data from: {self.last_page} pages. Please wait...")
        else:
            print(f"Scraping data from: {self.last_page} pages. Please wait...")
        ...
        try:
            ...
            with open(
                ...
                for page_num in range(1, self.last_page + 1):
                    ...
                    urls = [url['href'] for url in soup_page.find_all("a", class_="td-image-wrap")]
                    ...
                    for url in urls:
                        ...
                        published_date = data_page.select_one("time.entry-date.updated.td-module-date")
                        ...
                        content = [content.text.strip() for content in
                                   data_page.select("div.td-post-content.tagdiv-type p")]

                        # author = data_page.find("div", class_="td-post-author-name").find("a")
                        # author = author.text.strip() if author else ""
                        # published_date = data_page.find("time",
                        #                                 class_="entry-date updated td-module-date")
                        # published_date = published_date.text.strip()  if published_date else ""
                        # category = data_page.find("li", class_="entry-category").find("a")
                        # category = category.text.strip() if category else ""
                        # title = data_page.find("h1", class_="entry-title")
                        # title = title.text.strip() if title else ""
                        # content = [content.text.strip() for content in
