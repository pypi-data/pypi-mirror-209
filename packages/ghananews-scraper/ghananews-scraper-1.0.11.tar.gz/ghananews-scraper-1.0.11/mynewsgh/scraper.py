...
class MyNewsGh:
    def __init__(self, url: str, limit_pages: int = None):
        ...
        self.pages = soup.find_all("li", class_="mvp-blog-story-wrap left relative infinite-post")

        if self.limit_pages is not None:
            print(f"Scraping from: {self.limit_pages} pages. Please wait...")
            self.page_url_list = [self.url + f"page/{page}/" for page in range(1, int(self.limit_pages) + 1)]
        else:
            total_pages = soup.find("div", class_="pagination").span.text.split(" ")[-1]
            print(f"Scraping from: {int(total_pages)} pages. Please wait...")
            self.page_url_list = [self.url + f"page/{page}/" for page in range(1, int(total_pages) + 1)]
                ...
                with requests.Session() as session:
                    for page_url in self.page_url_list:
                        ...
                        for url in articles:
                            ...
                            # title
                            title = soup_page.find("h1", class_="mvp-post-title left entry-title")
                            title = title.text.strip() if title else ""
                            # published data
                            published_date = soup_page.find("time", class_="post-date updated")
                            published_date = published_date.text.strip() if published_date else ""
                            # author
                            author = soup_page.find("span", class_="author-name vcard fn author").find("a")
                            author = author.text.strip() if author else ""
                            # content
                            content = [content.text.strip() for content in
                                       soup_page.find("div", {"id": "mvp-content-main"}).find_all("p")]
...
#     news = MyNewsGh(url="https://www.mynewsgh.com/category/world/", limit_pages=5)
#     news.download()
