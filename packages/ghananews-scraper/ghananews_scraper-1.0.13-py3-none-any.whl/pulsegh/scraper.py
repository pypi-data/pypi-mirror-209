...
class PulseGh:
    def __init__(self, url: str, total_pages: int = None, limit_pages: int = None):
        ...
        # if self.total_pages is not None and self.limit_pages is None:
        #     logger.info(f"Scraping from: {self.total_pages} pages. Please wait...")
        # elif self.limit_pages is not None and self.total_pages is None:
        #     logger.info(f"Scraping from: {self.limit_pages} pages. Please wait...")
        #     self.total_pages = self.limit_pages
        # elif self.limit_pages is not None and self.total_pages is not None:
        #     if (self.total_pages == self.limit_pages) or (self.total_pages > self.limit_pages):
        #         self.total_pages = self.limit_pages
        #         logger.info(f"Scraping from: {self.limit_pages} pages. Please wait...")
        # elif self.total_pages is not None and self.limit_pages is not None:
        #     if self.total_pages < self.limit_pages:
        #         logger.error(f"total pages:{self.total_pages} less than limited pages:{self.limit_pages}")
        #         sys.exit(1)
                ...
                with requests.Session() as session:
                    ...
                    for page_num in range(0, self.total_pages + 1):
                        ...
                        for page in page_urls:
                            ...
                            # category = response_page.find("a", class_="breadcrumbs-link").find("span", class_="breadcrumbs-item")
...
#     news = PulseGh(url="https://www.pulse.com.gh/sports/football", total_pages=100, limit_pages=None)
#     news.download()
