...
def main(**kwargs):
    ...
    if kwargs['mode'] == 'condor':
        ...
        for i, segment in enumerate(segment_list):
            ...
            os.system('chmod 777 '+config["mlyPipelineSource"]+'/offline_search.py')
    
            thearguments = ("--mode="+'job'
                            +" --i="+str(i)
                            )

            public_files = ",".join(list(dict.fromkeys(list(frames_dir.values()))))+','+config["mlyPipelineSource"]

            job = Job(name = jobname
                    ,executable = config["mlyPipelineSource"]+"/offline_search.py"
                    ...
                    ,extra_lines=["accounting_group_user="+config['accounting_group_user']
                                    ...
                                    # ,"initialdir = "+config['path']
                                    # ,"when_to_transfer_output = ON_EXIT"
                                    # #,"transfer_input_files ="
                                    # ,"public_input_files = "+public_files
        ...
        td_pe_model = load_model(config["td_pe_model_path"]) # Load time-domain PE model
        fd_pe_model = load_model(config["fd_pe_model_path"]) # Load frequency-domain PE model
        ...
        for j in range(int(segment_size/config["duration"] - (config["required_buffer"] - config["duration"]) )):
            ...
            if mly_output["scores"]["combined"] >= threshold:
                ...
                mly_output = runTimeFrequencyParameterEstimation(
                    thepod, td_pe_model, fd_pe_model, mly_output
                )
