...
_BFS_IGNORE_TYPES = (types.ModuleType, type(None), bool, float, six.binary_type,
                     ...
                     types.BuiltinMethodType, types.GeneratorType) + six.integer_types

if sys.version_info >= (3, 5, 0):
  _BFS_IGNORE_TYPES += (types.CoroutineType, )

# Maximum length of lists to traverse. This is to prevent lists from being traversed,
# since with a good chance, lists that contain code objects are short.
_MAX_LIST_LEN = 100
...
def GetCodeObjectAtLine(module, line):
  ...
  for code_object_node in _GetModuleCodeObjects(module):
    lines = sorted(list(_GetLineNumbers(code_object_node.obj)))

    for co_line_number in lines:
      if co_line_number == line:
        return (True, code_object_node)
...
class BFSNode(object):
  ...
  def __repr__(self):
    return "<BFSNode> " + repr(self.obj) + " ;;; " + repr(self.parent) + " ;;; " + repr(self.grandparent)

  def get_id(self):
    if type(self.parent) is tuple:
      return id(self.grandparent)
...
def _GetModuleCodeObjects(module):
  ...
  current = [BFSNode(module, None, None)]
  code_objects_nodes = set()
  while current:
    current = _FindCodeObjectsReferents(module, current, visit_recorder)
    code_objects_nodes |= current
    ...
    current = [BFSNode(code_object.obj.co_consts, code_object.obj, code_object.parent) for code_object in current]

  return code_objects_nodes
...
def _FindCodeObjectsReferents(module, start_objects, visit_recorder):
  ...
  def CheckIgnoreList(lst):
    """Returns True if the list can be ignored. A list is ignored if it is too long"""
    if not hasattr(module, '__file__'):
      return False

    return len(lst) > _MAX_LIST_LEN
  ...
  while current and depth < _MAX_REFERENTS_BFS_DEPTH:
    ...
    for current_obj in current:
      referents = gc.get_referents(current_obj.obj)
      if (current_obj.obj is not module.__dict__ and
      ...
      for obj in referents:
        try:
          ...
          if isinstance(obj, list) and CheckIgnoreList(obj):
            continue

          if isinstance(obj, six.class_types) and CheckIgnoreClass(obj):
            continue

          if isinstance(obj, types.CodeType) and not isinstance(current_obj.obj, dict):
            code_objects.add(BFSNode(obj, current_obj.obj, current_obj.parent))
          else:
            new_current.append(BFSNode(obj, current_obj.obj, current_obj.parent))

        except ReferenceError:  # As we are using weak references to iterate, they can disappear
          pass
