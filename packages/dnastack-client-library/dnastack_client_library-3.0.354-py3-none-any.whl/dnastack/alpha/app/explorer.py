...
class _Collection:
    ...
    def __init__(self, factory: EndpointRepository, cs: CollectionServiceClient, collection: Collection, no_auth: bool):
    ...
    def blobs(self, *, ids: Optional[List[str]] = None, names: Optional[List[str]] = None) -> Dict[str, Optional[Blob]]:
        ...
        id_to_name_map: Dict[str, str] = SimpleStream(
            self.query(f"SELECT id, name FROM ({self._collection.itemsQuery}) WHERE {conditions}")
...
class Explorer:
    ...
    def collection(self, id_or_slug_name: Optional[str] = None, *, name: Optional[str] = None) -> _Collection:
        ...
        stream = SimpleStream(self._cs.list_collections(no_auth=self._no_auth))
        if id_or_slug_name is not None:
            assert id_or_slug_name.strip(), 'The ID or slug name cannot be empty.'
            stream.filter(lambda endpoint: id_or_slug_name == endpoint.id or id_or_slug_name == endpoint.slugName)
        elif name is not None:
            assert name.strip(), 'The name cannot be empty.'
            stream.filter(lambda endpoint: name == endpoint.name)
        else:
            raise NotImplementedError()

        target_collections = stream.to_list()
        if len(target_collections) == 1:
            return _Collection(self._factory, self._cs, target_collections[0], no_auth=self._no_auth)
