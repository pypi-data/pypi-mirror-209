...
#          Exclude Todays Date
# ------------------------------
def remove_todays_data(df):
    """Function to remove data for today's date from the provided DataFrame.
    
    Args:
        df (pandas.DataFrame): DataFrame to be processed.
    
    Returns:
        pandas.DataFrame: Processed DataFrame with today's data removed.
    """
    df['date'] = pd.to_datetime(df['date']) # Ensure the 'date' column is of datetime type
    return df[df['date'].dt.date != datetime.today().date()]