...
class DjangoCreate:
        ...
        for field in self.values:
            ...
            if "default" in field.schema:
                ...
                if repr(default_schema["value"]) == field.value_repr:
                    continue
                if (
                    "type" in default_schema
                    and default_schema["type"] == "Decimal"
                    and field.value == Decimal(default_schema["value"])
                ):
                    continue
    ...
    def template_parts(self):
        ...
        defaults = {self.primary_key.name: self.primary_key.value}
