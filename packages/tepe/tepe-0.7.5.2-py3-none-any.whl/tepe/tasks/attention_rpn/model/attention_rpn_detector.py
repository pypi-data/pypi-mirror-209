...
class AttentionRPNDetector(QuerySupportDetector):
    ...
    def forward_model_init(self,
        ...
        # support shape: [1, 3, 320, 320]

        feats = self.extract_support_feat(img)
        # rois = bbox2roi([bboxes for bboxes in gt_bboxes])
        num_imgs = len(img)
        gt_bboxes = [torch.tensor([[0., 0., 320., 320.]], dtype=torch.float32)] * num_imgs
        rois = bbox2roi([bboxes for bboxes in gt_bboxes]).to(img.device) #.cuda()
        res4_roi_feat = self.rpn_head.extract_roi_feat(feats, rois)  # [1, 1024, 14, 14]
        res5_roi_feat = self.roi_head.extract_roi_feat(feats, rois)  # [1, 2048, 7, 7]
        ...
        return res4_roi_feat, res5_roi_feat
        # return {
        #     'gt_labels': gt_labels,
    ...
    def model_init(self) -> None:
        ...
        res4_roi_feats = torch.cat(
            self._forward_saved_support_dict['res4_roi_feats'])  # [1, 1024, 14, 14]
        # used for multi relation head
        res5_roi_feats = torch.cat(
            self._forward_saved_support_dict['res5_roi_feats'])  # [1, 1024, 7, 7]
    ...
    def forward(self,
        ...
        """
        if torch.onnx.is_in_onnx_export() and not self.is_model_init:
            return self.init_onnx_export(img[0])
        if torch.onnx.is_in_onnx_export() and self.is_model_init:
            assert len(img_metas) == 1
            return self.onnx_export(img[0], img[1], img[2], img_metas)
        if mode == 'train':
    ...
    def simple_init(self,
        ...
        _forward_saved_support_dict = {
            'gt_labels': [],
            'res4_roi_feats': [],
            'res5_roi_feats': []
        }
        _forward_saved_support_dict['gt_labels'].extend(gt_labels)
        _forward_saved_support_dict['res4_roi_feats'].append(
            res4_roi_feat)
        _forward_saved_support_dict['res5_roi_feats'].append(
            res5_roi_feat)

        self.gt_labels = torch.cat(_forward_saved_support_dict['gt_labels'])
        self.class_ids = set(self.gt_labels.data.tolist())
        # used for attention rpn head
        self.res4_roi_feats = torch.cat(
            _forward_saved_support_dict['res4_roi_feats'])  # [1, 1024, 14, 14]
        # used for multi relation head
        self.res5_roi_feats = torch.cat(
            _forward_saved_support_dict['res5_roi_feats'])  # [1, 1024, 7, 7]

        # set the init flag
        self.is_model_init = True
        # clear support dict
        for k in _forward_saved_support_dict.keys():
            _forward_saved_support_dict[k].clear()
    ...
    def simple_test(self,
                    ...
                    rescale: bool = False,
                    res4_roi_feats: Tensor = None,
                    res5_roi_feats: Tensor = None) -> List[List[np.ndarray]]:
        ...
        # query shape: [1, 3, 563, 1000]

        if res4_roi_feats is not None:
            self.res4_roi_feats = res4_roi_feats
        if res5_roi_feats is not None:
            self.res5_roi_feats = res5_roi_feats

        results_dict = {}
        query_feats = self.extract_feat(img)
        for class_id in self.class_ids:
            support_res4_roi_feat = \
                self.res4_roi_feats[self.gt_labels == class_id].mean([0, 2, 3], True)  # [1, 1024, 1, 1]
            support_res5_roi_feat = \
                self.res5_roi_feats[self.gt_labels == class_id].mean([0], True)  # [1, 2048, 7, 7]
            print(type(img_metas[0]['img_shape']))
    ...
    def init_onnx_export(self,
        ...
        num_imgs = 1
        gt_bboxes = [torch.tensor([[0., 0., 320., 320.]], dtype=torch.float32)] * num_imgs
        rois = bbox2roi([bboxes for bboxes in gt_bboxes]) #.cuda()
        res4_roi_feat = self.rpn_head.extract_roi_feat(feats, rois)  # [1, 1024, 14, 14]
        res5_roi_feat = self.roi_head.extract_roi_feat(feats, rois)  # [1, 2048, 7, 7]
    ...
    def onnx_export(self,
        ...
        # query shape: [1, 3, 563, 1000]
        results_dict = {}
        query_feats = self.extract_feat(img[0])


        support_res4_roi_feat = \
            res4_roi_feats.mean([0, 2, 3], True)  # [1, 1024, 1, 1]
        support_res5_roi_feat = \
            res5_roi_feats.mean([0], True)  # [1, 2048, 7, 7]

        proposal_list = self.rpn_head.simple_test(
            query_feats, support_res4_roi_feat, img_metas)

        results_dict[0] = self.roi_head.simple_test(
            query_feats,
            support_res5_roi_feat,
            proposal_list,
            img_metas,
