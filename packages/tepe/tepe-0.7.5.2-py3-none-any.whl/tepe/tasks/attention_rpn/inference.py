...
from .model.query_support_detector import QuerySupportDetector
from .model.attention_rpn_detector import AttentionRPNDetector
...
def build_detector(cfg, train_cfg=None, test_cfg=None):
    ...
    detector = AttentionRPNDetector(**cfg)
...
def process_support_images(model: nn.Module,
    ...
    data['gt_bboxes'] = [torch.tensor([[0., 0., 320., 320.]], dtype=torch.float32)] * len(data['gt_bboxes'].data[0])
    ...
    # from .onnx_test import onnx_inf
    # onnx_inf(data['img'])

    # initialize model
    with torch.no_grad():
        # data.keys(): ['img_metas', 'img', 'gt_bboxes', 'gt_labels']
        # result = model(mode='model_init', **data)

        model.simple_init(**data)
