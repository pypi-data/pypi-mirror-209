...
from .util import convert_avg_params, gather_results, mkdir
from .model.weight_averager import ExpMovingAverager
...
class TrainingTask(LightningModule):
    ...
    def __init__(self, cfg, evaluator=None):
        ...
        if self.cfg.ema:
            self.weight_averager = ExpMovingAverager(
                cfg.weight_averager.decay, device=self.device
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        # log train losses
        if self.global_step % self.cfg.print_interval == 0:
    ...
    def training_epoch_end(self, outputs: List[Any]) -> None:
        self.trainer.save_checkpoint(os.path.join(self.cfg.output_dir, "model_last.ckpt"))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx % self.cfg.print_interval == 0:
            lr = self.optimizers().param_groups[0]["lr"]
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        if all_results:
            eval_results = self.evaluator.evaluate(
                all_results, self.cfg.output_dir, rank=self.local_rank
            )
            metric = eval_results[self.cfg.save_key]
            # save best model
            if metric > self.save_flag:
                self.save_flag = metric
                best_save_path = os.path.join(self.cfg.output_dir, "model_best")
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        if all_results:
            ...
            json_path = os.path.join(self.cfg.output_dir, "results.json")
            ...
            if self.cfg.test_mode == "val":
                ...
                txt_path = os.path.join(self.cfg.output_dir, "eval_results.txt")
                with open(txt_path, "a") as f:
                    for k, v in eval_results.items():
                        f.write("{}: {}\n".format(k, v))
        else:
    ...
    def configure_optimizers(self):
        ...
        optimizer = build_optimizer(
            params=self.parameters(), lr=self.cfg.learning_rate, **optimizer_cfg)

        schedule_cfg = copy.deepcopy(self.cfg.lr_schedule)
    ...
    def optimizer_step(
        ...
        """
        # warm up lr
        if self.trainer.global_step <= self.cfg.warmup.steps:
            warmup_name = self.cfg.warmup.name
            if warmup_name == "constant":
                warmup_lr = (
                    self.cfg.learning_rate * self.cfg.warmup.ratio
                )
            elif warmup_name == "linear":
                k = (1 - self.trainer.global_step / self.cfg.warmup.steps) * (
                    1 - self.cfg.warmup.ratio
                )
                warmup_lr = self.cfg.learning_rate * (1 - k)
            elif warmup_name == "exp":
                k = self.cfg.warmup.ratio ** (
                    1 - self.trainer.global_step / self.cfg.warmup.steps
                )
                warmup_lr = self.cfg.learning_rate * k
            else:
    ...
    def on_pretrain_routine_end(self) -> None:
        if self.cfg.ema:
            ...
            if self.weight_averager and self.weight_averager.has_inited():
                ...
                return
            self.weight_averager = ExpMovingAverager(
                self.cfg.weight_averager.decay, device=self.device
            )
