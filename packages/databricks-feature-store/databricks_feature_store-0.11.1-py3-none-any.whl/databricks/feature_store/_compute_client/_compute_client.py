...
class ComputeClient:
    ...
    def _create_table(
        ...
        *,
        ...
        is_uc_table = uc_utils.is_uc_table(name)
    ...
    def register_table(
        ...
        *,
        ...
        is_uc_table = uc_utils.is_uc_table(delta_table)
    ...
    def drop_table(self, name: str) -> None:
        ...
        if not feature_table_exists and not delta_table_exist:
            if not uc_utils.is_uc_table(name):
    ...
    def set_feature_table_tag(self, *, table_name: str, key: str, value: str) -> None:
        if uc_utils.is_uc_table(table_name):
    ...
    def _check_pk_tk_constraint(
        ...
        # Get existing PK of the table from information_schema
        existing_pk = self._spark_client.get_pk_from_table_info_schema(full_table_name)
        if existing_pk:
            # Check if existing PK constraint match with expected PK constraint
            if not expected_pk == existing_pk:
                raise ValueError(
                    f"Existing Primary Key constraint {existing_pk} for Delta table '{full_table_name}' does not "
                    f"match with expected Primary Key constraint {expected_pk}. "
                    f"Use 'ALTER TABLE {full_table_name} DROP PRIMARY KEY;' to drop existing PK constraint and "
                    f"'ALTER TABLE {full_table_name} ADD CONSTRAINT {table_name}_pk PRIMARY KEY({', '.join(expected_pk)})' "
            ...
            if expected_pk:
                ...
                raise ValueError(
                    f"Expect Primary Key constraint {expected_pk} for Delta table '{full_table_name}' but found none. "
                    f"Use 'ALTER TABLE {full_table_name} ADD CONSTRAINT {table_name}_pk PRIMARY KEY({', '.join(expected_pk)})' "
                    f"to add expected Primary Key constraint on the Delta table."
                )
