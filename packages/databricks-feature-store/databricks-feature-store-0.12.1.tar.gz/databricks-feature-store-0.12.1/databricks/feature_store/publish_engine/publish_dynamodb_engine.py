...
class PublishDynamoDBEngine(abc.ABC):
    ...
    def drop_table(self):
        """
        Drop/Delete table from DynamoDB account
        """
        table = self._dynamodb_resource.Table(self.table_name)
        table.delete()
        # DynamoDB delete request is async, wait until request finishes to make synchronous
        # and follow same pattern as CosmosDB. With synchrnous call, user will be able to
        # immediately recreate table with same name following response from `drop_online_table` request.
