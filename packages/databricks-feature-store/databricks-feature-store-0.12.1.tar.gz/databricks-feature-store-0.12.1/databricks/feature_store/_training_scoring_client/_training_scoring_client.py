...
class TrainingScoringClient:
    ...
    def create_training_set(
        ...
        # Excluded columns that are on-demand inputs should be in feature_spec.yaml with include=False.
        # TODO (ML-29937): Excluded columns that are lookup keys should be in feature_spec.yaml with include=False.
        column_infos = [
            ci
            for ci in column_infos
            if ci.include or ci.output_name in on_demand_input_names
        ]
    ...
    def score_batch(self, model_uri: str, df: DataFrame, result_type: str) -> DataFrame:
        ...
        extra_df_columns = [
            col for col in df.columns if col not in feature_spec_columns
        ]
    ...
    def _get_and_validate_lookback_windows(
        ...
        for fl in feature_lookups:
            table_lookback_windows_map[fl.table_name].add(fl.get_lookback_window())
