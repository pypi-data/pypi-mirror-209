...
class FeatureLookup(_FeatureStoreObject):
    ...
    :param lookup_key: Key to use when joining this feature table with the :class:`DataFrame <pyspark.sql.DataFrame>` passed to
       ...
       in the DataFrame passed to :meth:`.FeatureStoreClient.create_training_set`. The type and order of
    ...
    def __init__(
        ...
        **kwargs,
        ...
        lookback_window = kwargs.pop("lookback_window", None)
        ...
        if lookback_window is not None:
            if not isinstance(lookback_window, datetime.timedelta):
                raise ValueError(
                    f"Unexpected type for lookback_window: {type(lookback_window)}, only datetime.timedelta allowed."
                )
            if lookback_window < datetime.timedelta(0):
                raise ValueError(
                    f"Unexpected negative value for lookback_window: {lookback_window}, only positive values allowed."
                )
