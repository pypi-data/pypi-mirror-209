...
class SparkClient:
    ...
    def get_pk_from_table_info_schema(self, full_table_name: str) -> List[str]:
        """
        Get primary keys of a table from information_schema.
        """
        ...
        return (
            ...
            WHERE c.table_catalog = '{catalog_name.lower()}'
              AND c.table_schema = '{schema_name.lower()}'
              AND c.table_name = '{table_name.lower()}'
    ...
    def set_cols_not_null(self, full_table_name: str, cols: List[str]):
        ...
        for c in cols:
            self._spark.sql(
                f"ALTER TABLE {full_table_name} ALTER COLUMN {sanitize_identifier(c)} SET NOT NULL"
            )
    ...
    def set_pk_statement(
        ...
        return f"ALTER TABLE {full_table_name} ADD CONSTRAINT {pk_name} PRIMARY KEY({', '.join(sanitize_identifiers(pk_cols))})"
